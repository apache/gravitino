...
>
> Epic: [#10238](https://github.com/apache/gravitino/issues/10238) — *Optimize tree lock as it's not function well in distribution mode* (supersedes [#10474](https://github.com/apache/gravitino/issues/10474), closed)
>
> **Baseline.** The analysis was first written against `main` in July 2026 and re-checked against `main` at `03da573642` (2026-09-15). Where the two differ, the text says what has changed. Code references without a commit are to the 2026-09-15 baseline.
...
5. **Strong consistency for Gravitino-managed metadata.** For every entity whose only source of truth is the Gravitino store (metalake, catalog, managed schema, fileset, model, view, function, user, group, role, tag, policy, job, and the relations between them), two servers sharing one database must never lose an update, create two winners, leave a child under a deleted parent, or attach a relation to a deleted endpoint. This is the bar the database can meet, so it is the bar we set.
6. **Simple to implement and to maintain.** Prefer a rule that is easy to read in one meta service over a mechanism that is theoretically stronger but hard to reason about. Where a guarantee would need a new subsystem, a long-held lock across an RPC, or a protocol that only its author can review, write down the gap instead and accept it as a documented limit.
...
3. **Stale reads are a different problem.** This document is about concurrent *writes*. Keeping each server's cached reads fresh is the `EntityChangeLogPoller` work, not this. The implementation design does list the read contract that the write rules assume (which reads may be stale, which must not), but it does not design a new cache.
4. **We do not change how external catalogs behave.** Hive, Iceberg and JDBC keep their own concurrency semantics, and Gravitino does not promise more than they give. In particular, an external system that only offers whole-object replace (HMS, Glue) keeps its lost-update behaviour; Gravitino documents it, it does not paper over it.
5. **No exactly-once across the external system and the store.** A crash or an ambiguous timeout between the external call and the store write can leave the two sides different. Recovery is by reconcile and idempotent retry where the external API allows it, not by a promise of atomic commit.
...
Here is how the self-fix works, traced through the code. If create succeeds in the external system but the store write fails, the error is **hidden** — `internalCreateTable:688-696` catches it, logs it, and still returns success to the client with no stored entity. Nothing is fixed until someone reads the table again. On that next read, `loadTable:142` sees `imported == false` and runs `importTable`, which writes the entity into the store. Schemas behave the same way (`internalLoadSchema` + `importSchema`). So a failed store write **does** auto-correct on the next load — with two limits that the rest of this document keeps coming back to. The next load rebuilds only what the external system can tell it: the entity row and its columns. It cannot bring back **attachments** (owner, tags, policies, role grants, statistics) that were lost in between, it cannot recover a modification that a later external write overwrote, and for an id-less catalog it cannot tell "the same table" from "a different table that reused the name". "The next read fixes it" therefore means "the next read re-syncs the reconstructible part", nothing more.
...
A lock does not fix this: a fencing token has nothing to validate against on HMS, and it would only serialize callers going through Gravitino while Spark, Trino and the Hive CLI write to the same HMS directly. Nor does "re-read after `alter_table` and compare" fix it: a read-back can show that the winner's state differs from what this caller asked for, and can re-apply *this caller's* change list to the fresh base, but it cannot recover a modification that this caller never knew about, and another writer can land between the re-read and the re-apply. Reading back is therefore a **detection and mirroring** step — it keeps the store row honest and can warn the caller — not a repair. The only real fix is a conditional write in the backend itself: Hive 4's `AlterTableRequest.expectedParameterKey/Value` is a single-parameter CAS, but Gravitino builds against Hive `2.3.9`/`3.1.3`, so it stays a future option. Until then this is a documented per-connector limitation of the two HMS-shaped connectors, listed as such in the implementation design.
...
| Two operations at the same time                 | What happens with no lock                                                                                                                                                                                                             | Level | Status on `main` (2026-09-15)                                                                                                                                                                                   |
|-------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| create × create                                 | the external catalog accepts only one; the loser gets `TableAlreadyExistsException` and never reaches the store                                                                                                                       | 0     | the external system decides; nothing to do                                                                                                                                                                      |
| import × import                                 | one wins; the other catches `EntityAlreadyExistsException` and reloads (`TableOperationDispatcher.loadTable`)                                                                                                                         | 0     | handles many servers already                                                                                                                                                                                    |
| import × drop                                   | a read that started before the drop writes the row back after it → left-over row                                                                                                                                                      | 1     | open — reconcile job [#12155](https://github.com/apache/gravitino/issues/12155)                                                                                                                                 |
| copy alter × copy alter                         | the copy can end up with the loser's value; the next `loadTable` reads the external system again and re-syncs it. On HMS and Glue the external object itself can lose a field, which is the separate connector defect described above | 0     | store side: each copy is a version-checked update, the loser's `OptimisticLockException` is logged and swallowed on purpose (`OperationDispatcher.operateOnEntity`) because the external change already happened |
| create child × drop parent                      | a child row can be added under a parent that was just deleted → left-over rows, invisible because list goes to the external system                                                                                                    | 1     | **closed in the store**: every child insert holds the schema row `FOR SHARE` in its transaction, `deleteSchema` holds the catalog row `FOR UPDATE` (`SchemaMetaService.lockSchemaForEntityWrite`)              |
| rename × drop                                   | drop deleted the store row **without checking anything**, so a table alive under its new name lost its row **and all its attachments**                                                                                                | 2     | **fixed** by [#12232](https://github.com/apache/gravitino/issues/12232): a `false` from the external drop leaves the store alone; rename updates by id and reports a 0-row update as an error                    |
| **drop × create, same name**                    | drop's store step resolves the name **after** the external drop, so it can delete the registration of a table that was re-created under that name in between (see E1 below)                                                          | **2** | **open** — the service deletes with a version check on `(id, current_version)` now, but the dispatcher hands it a name, not the identity it observed before the external call                                   |
| crash between the external call and `store.put` | a left-over object in the external system                                                                                                                                                                                             | 1     | no lock helps — this is a crash, not concurrency; reconcile [#12155](https://github.com/apache/gravitino/issues/12155)                                                                                          |

**E1 in detail — the version check works, but the identity is picked up too late.** `dropTable` calls the external drop and, when it returns `true`, calls `store.delete(ident, TABLE)`. `TableMetaService.deleteTable` then reads whatever row currently sits under that name and deletes it with a CAS on its version. The CAS is correct for the row it reads; the problem is *which* row that is:

```text
A: external drop(t) succeeds            (old incarnation of t)
A: pauses before store.delete
B: creates a new external t, its registration lands in the store (new id, or the old row upserted by name)
A: store.delete(t) reads the row now under t → CAS passes → the live registration and its attachments are gone
```

The fix is to capture the registration identity (id and version) **before** the external call and pass it into a conditional delete, so that a mismatch is a conflict instead of a deletion. The same audit applies to rename, purge and schema drop, and to `operateOnEntity`, which today compares the returned id with the external id only *after* `store.update` has committed.
...
**What this table says: the concurrency cases are level 0 or 1, except the ones where the store write picks the wrong target.** Those are races that turn destructive because the write resolves its target by *name* at the wrong moment — an identity mistake is exactly how an ABA race becomes a data loss. TreeLock hides them on one server and does nothing about them once several servers run. One of the two is fixed on `main` (#12232); the other (E1) is open.
...
The last outcome only partly fixes itself. The next `loadTable(t2)` re-imports the entity row and its columns, but drop's store step cascades: `TableMetaService.deleteTable` also soft-deletes everything attached to that table id — **owner, tag, policy, role grants, statistics** — and `importTable` restores none of them, not even when the id is reused. So the relations are silently and irreversibly gone, while the caller is told the rename succeeded. The fix is simple: **do not touch the store when the external drop returned `false`**, since `false` here means "still there under another name", and leave stale-row cleanup to reconcile. Tracked as [#12232](https://github.com/apache/gravitino/issues/12232), **merged on `main`** (#12235): the dispatcher keeps the registration on `false`, and a rename whose store update matches 0 rows raises `GravitinoRuntimeException` instead of reporting success.

The missing-row case matters only across nodes, or after removing TreeLock. Today TreeLock takes a write lock on the schema for both drop and rename, so on a single server they cannot interleave; under HA they already can, because the lock is per-node. One remaining gap sits outside the dispatcher: `TableHookDispatcher.dropTable` calls `authorizationPluginRemovePrivileges` even when the dispatcher returned `false`, so the plugin side of this race is not covered by #12232 and is listed separately in the implementation design.

A second concurrent case is **create a child while its parent is being dropped** — `createTable(db.t1)` on server A while `dropSchema(db)` on server B. For external-backed catalogs the external system decides this race. But for entities whose only source of truth is the Gravitino store, and for Gravitino's own rows, nothing stops server A from inserting the child row just after server B removed the parent — an **orphan child**. A per-row version check cannot express this. A plain `INSERT ... SELECT ... WHERE parent.deleted_at = 0` is also insufficient at Gravitino's `READ_COMMITTED` isolation: drop can check that there are no children, the create can then observe a live parent and commit, and the drop can finally delete the parent. The shared parent-row transaction rule is what closes this, and on `main` it is in place from metalake down to the entity level: child inserts take the parent row `FOR SHARE`, parent drops take it `FOR UPDATE`, and the non-cascade "is it empty" check runs inside the drop transaction after the delete (`SchemaMetaService.deleteSchema`). What is still outside the transaction is the *manager-level* precondition: `CatalogManager.dropCatalog(force = false)` lists user-created schemas first and later calls `store.delete(..., cascade = true)`, so a schema created in between is cascaded instead of rejected. The implementation design lists this under lifecycle preconditions.

### A note on stale store rows

A natural follow-up: can the store keep a row that points at an external object that is already gone (a "stale row"), for example after an out-of-band drop in Hive or a failed `store.delete`? Yes, but for the entity itself it does not matter — relying on the external state is reliable here: `loadTable` asks the external system first, so a stale row is never trusted, and a later create with the same name overwrites it (`store.put(overwrite = true)`). Note what that overwrite does on MySQL: the upsert matches the natural key `(schema_id, table_name, deleted_at)` and **keeps the old row's `table_id`**, so the new external table inherits the stale row's identity — and with it whatever attachments were still on that id. On PostgreSQL the same statement conflicts on `table_id` instead and a different-id natural-key collision fails. The two backends do not agree, which is one more reason the create path should be a strict insert and the import path a separate, explicit intent.
...
### The really bad cases are about identity — with or without concurrency

Both cases below come from what the promise above already allows: a user changing the external system directly. Both are level 2. Both happen today, with TreeLock in place, on a single server, and no lock of any kind would change them. They share their root cause with E1 above: a store write resolves its target by name or by a copied id and lands on the wrong row.
...
- The concurrent rename/drop race ends either consistent or missing a row. The missing row is **not** repaired by the next read: `importTable` re-writes only the entity row, while the drop already cascaded the attachments, so owner, tags, policies, role grants and statistics are gone for good. This is level 2; [#12232](https://github.com/apache/gravitino/issues/12232) fixed it on `main`. The drop-after-recreate variant (E1) has the same shape and is still open.
...
- The really bad cases (S1, S2, E1 and the rename/drop bug) are about **identity**: a row that ends up pointing at the wrong object, or a delete that picks the wrong target. Two of them are plain races that become destructive because the target is resolved by name at the wrong moment; two happen with no concurrency at all. None of them is fixed by a lock once several servers run, and all of them are fixed by carrying the observed identity into a conditional store write.
...
| Correct across servers                   | Yes only after gates G1–G6 pass — the database is shared and is the judge                                                                             | Yes if the distributed lock is correctly implemented and every caller participates                                                                                                                                  |
...
The analysis above answers "can the database replace the lock for entity rows?". The review asked a sharper question: **once the database rules are in place, is anything left that TreeLock protects today and the database cannot take over?** This section answers it by going through the 161 `doWithTreeLock` call sites on `main` (2026-09-15) instead of the entity model, and marks every case the store cannot handle.

First, what the lock actually does. `TreeLockUtils.doWithTreeLock(ident, type, fn)` takes a **read** lock on every ancestor of `ident` and the requested read or write lock on `ident` itself, all inside one JVM. So in one process it can coordinate a parent operation against its children (drop schema vs. create table), and it can order the two halves of a two-store write (external call then store write) against another such write on the same subtree. It cannot see another server, a client writing to Hive directly, a crash between the two halves, or a stale cache on another node.

| Scenario                                                                        | Does TreeLock protect it today?                                                                  |
|---------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|
| create / drop / rename of a table on **one** node                               | yes — through the schema or catalog write lock                                                   |
| two ordinary `alterTable` on one node                                           | no — both take a READ lock                                                                       |
| two `alterFileset` on one node                                                  | yes — the dispatcher uses WRITE (the July draft of this document said READ; that was wrong)      |
| the same object from **two** nodes                                              | no                                                                                               |
| Spark / Trino / Hive CLI writing to the external system                         | no                                                                                               |
| a crash after the external call and before the store write                      | no                                                                                               |
| a stale entity cache on another node                                            | no                                                                                               |
| authorization hooks that run after the dispatcher returns (`*HookDispatcher`)   | no — they run outside the lock                                                                   |

So "TreeLock cannot give multi-node consistency" is true, and it is *not* the same as "removing it changes nothing": on a single server it still hides a handful of races that HA exposes today. The point of the removal plan is to close those races in the database first, so that single-server and multi-server behaviour become the same *good* behaviour, not the same bad one.

The labels below drive the plan. `REPLACED` means `main` already has a replacement and the lock is redundant there. `MUST REPLACE FIRST` means a replacement has to be merged before the lock is touched. `NO CHANGE` means the lock does not protect the case even today.

### Group A — Java object lifetime: the database cannot express this

| #  | Case                                                        | What TreeLock did                                                                                                                                                       | Status on `main`                                                                                                                                                                                                                                                                                           | Label                                                                        |
|----|-------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------|
| A1 | `dropCatalog` against any running operation on that catalog | a **WRITE lock on the metalake** kept every read-locked operation under that metalake out of this JVM while the cache entry was removed and `CatalogWrapper.close()` ran | **Replaced by operation leases** ([#12403](https://github.com/apache/gravitino/issues/12403)): every operation takes `CatalogWrapper.tryAcquire()`, the cache removal listener only `retire()`s the wrapper, and the catalog and its `IsolatedClassLoader` are cleaned up once by the last `release()`      | **REPLACED** — keep the lease tests; audit the few unleased readers          |
| A2 | `alterCatalog` (including rename) against running operations | same metalake write lock; the old wrapper was closed and a new one built                                                                                                | same lease mechanism                                                                                                                                                                                                                                                                                       | **REPLACED**                                                                 |
| A3 | `dropCatalog(force = true)` on a managed catalog             | under the metalake write lock it calls the connector's `dropSchema` for every schema, so real external work happens while the whole metalake is blocked in this JVM     | unchanged; the store side is covered by the catalog-row `FOR UPDATE` and the schema-row `FOR SHARE`, but nothing stops a concurrent `createFileset` on another node from creating a directory after the cascade passed that schema                                                                         | **NEEDED, not a blocker** — a durable `DELETING` state on the catalog row before enumerating children; see the implementation design |

This is why the goal has always been "keep a small in-process helper where a Java object's lifetime needs one", not "delete every lock".

### Group B — a second external system that is never read back

For catalog metadata the store copy is re-synced on the next read (import). Other external write targets are never read back, so a wrong order stays wrong.

| #  | Case                                                                                                    | Order today                                                                                                                                                                            | What happens once the lock is gone                                                                                                                                                                        | Label                                                                                                                                                                       |
|----|---------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| B1 | Authorization plugin (Ranger and others) on `grantRolesToUser`, `revokeRolesFromUser` and the group and role versions | the whole call runs under a **write lock on the user, group or role** (`AccessControlManager`), and inside it the store is written first and the plugin second (`PermissionManager`) | two calls can reach the store in one order and Ranger in the other. The store then says "granted" while Ranger says "revoked", **and it stays that way** — nothing reads Ranger back                        | **MUST REPLACE FIRST** — carry the store version (or a generation) into the plugin call and let the newer one win; or keep a small per-principal in-process guard and write down the multi-server gap |
| B2 | HMS and Glue whole-object `alter_table`                                                                 | TreeLock takes a **read** lock for a normal alter, so two alters on one table already overlap in one JVM                                                                                | no change: a field the winner never edited can still be lost. A lock never fixed this, and a cross-node lock would not either, because Spark, Trino and the Hive CLI write to the same HMS directly       | **NO CHANGE** — a documented connector limitation until the backend offers a conditional write                                                                              |
| B3 | Managed fileset: the store row against the directory in HDFS or S3                                      | **drop is now row-first**: `store.deleteAndGet` wins the CAS, then removes the directories inside the same transaction, so a stale loser never touches storage. **Create is still directory-first**: `mkdirs` runs before `store.put(…, overwrite = true)` | the rows are ordered by the version check; storage is not: a losing create leaves an empty directory, a partial multi-location delete followed by a rollback leaves a live row over half-missing data | **NEEDED, not a blocker** — strict insert on create, then a `CREATING/ACTIVE/DELETING` state on the row so cleanup is idempotent; physical data already deleted cannot be restored by metadata OCC |
| B4 | Jobs against the external job executor                                                                  | `runJob` submits to the executor **before** `store.put`; `cancelJob` and the status poller now use version-checked updates and defer a lost CAS to the next poll ([#12669](https://github.com/apache/gravitino/issues/12669), [#12992](https://github.com/apache/gravitino/issues/12992)) | a job accepted by the executor whose registration fails, or whose server dies, is not undone; cancel and status can still reach the executor in the other order                                        | **NEEDED, not a blocker** — a durable submit intent plus executor idempotency; the executor is the source of truth when they disagree                                        |
| B5 | Hook-dispatcher privilege removal on drop                                                               | runs **outside** the lock, after the dispatcher returns, and today it runs even when the dispatcher returned `false`                                                                    | the #12232 fix keeps the store row on a `false` drop, but the plugin privileges are removed anyway                                                                                                        | **NEEDED, not a blocker** — skip the plugin call on `false`; give plugin events a stable identity so a stale event cannot remove a newer object's privileges                  |

### Group C — real gaps the database *can* close

This is the work list from July, re-checked against `main`. Most of it has landed.

| #  | Case                                                                                     | July finding                                                                                         | Status on `main` (2026-09-15)                                                                                                                                                                                                                                              |
|----|------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| C1 | no parent rule for `schema → table/fileset/model/topic/view/function`                    | only `metalake → catalog → schema` was covered                                                       | **done** — every child insert holds the schema row `FOR SHARE`; a table move to another schema holds the *new* schema; the lock order is `metalake → catalog → schema → entity`                                                                                            |
| C2 | metalake `enable` and `disable` write every catalog row outside a transaction            | `MetalakeManager` comment: "we can't make sure we can change all catalog properties in a transaction" | **open** — the comment and the loop are still there                                                                                                                                                                                                                        |
| C3 | the non-cascade "is it empty" check misses views and functions                           | `SchemaMetaService.checkSchemaIsEmpty`                                                               | **done** — the check covers descendant schemas, tables, filesets, models, topics, views and functions, and runs inside the drop transaction after the version-checked delete                                                                                                |
| C4 | drop deletes the store row by name                                                       | `TableOperationDispatcher` → `store.delete(ident)`                                                   | **done at the service level** — `deleteTable` reads the row and deletes it with a CAS on `(table_id, current_version)` before touching dependents; **open at the dispatcher level** — the identity is resolved after the external call (E1)                                 |
| C5 | rename against drop deletes a live entity's row and attachments                          | [#12232](https://github.com/apache/gravitino/issues/12232)                                           | **done** (#12235)                                                                                                                                                                                                                                                          |
| C6 | managed schema and managed fileset create with a blind upsert                            | `ManagedSchemaOperations` and `FilesetCatalogOperations`                                             | **half done** — managed schema is `put(…, false)`; managed fileset is still `exists()` then `put(…, true)`, so two concurrent creates end with the second one silently replacing the first (M1)                                                                          |
| C7 | model version counter, one live owner, tag/policy/statistic endpoints                    | counter raised in a later statement; `uk_ow_me_del` includes `owner_id`; endpoints looked up outside the transaction | **counter done** (`bumpModelVersionAndLatestVersion`); **tag and policy done on their own side** (tag/policy rows `FOR UPDATE`, metalake `FOR SHARE`); **open**: the metadata-object end of tag/policy/statistic relations is not fenced; owner still has no uniqueness per object and no endpoint fence; role membership inserts do not fence the role ([#13001](https://github.com/apache/gravitino/issues/13001)–[#13004](https://github.com/apache/gravitino/issues/13004)) |
| C8 | version checks not really in use: several entities kept the version fixed and compared whole rows | catalog, schema, topic, fileset, policy                                                              | **done** — every entity service uses `OccWriteSupport.updateWithVersion/deleteWithVersion`, versions advance on every write including overwrite (`ON DUPLICATE KEY UPDATE current_version = current_version + 1`), and a 0-row result maps to `OptimisticLockException` (HTTP 409, error code 1012) or `NoSuchEntityException` (404) |
| C9 | statistics                                                                               | not reviewed                                                                                         | **open** — `batchInsertStatisticPOsOnDuplicateKeyUpdate` is last-writer-wins with no version and no live-target check                                                                                                                                                         |
| C10 | manager-level lifecycle preconditions                                                   | not reviewed                                                                                         | **open** — `dropCatalog(force = false)` and `dropMetalake` check "in use" and "has children" in the manager, then call a cascade delete; a child created in between is cascaded instead of rejected                                                                          |

### Group D — TreeLock is not protecting these anyway

Worth listing, so the lock does not get more credit than it deserves:

- **Two `alterTable` calls on one entity** — read lock, so they already overlap.
- **Authorization plugin calls from the hook dispatchers** — after `dispatcher.dropTable(...)` returns, so outside the lock.
- **`setOwner`** — takes only a read lock on the principal, so two calls already race.
- **`PartitionOperationDispatcher`** — six call sites, none writes to the store.
- **Import on read** — `loadTable` writes to the store under a schema write lock that no other server can see.
- **Anything across servers** — by design.

### What the review concludes

1. The entity-row story holds and is now largely implemented: for external-backed catalogs the external system stays the source of truth, and for managed entities the database rules (strict insert, version-checked update and delete, parent row held in the child's transaction, cascade in one transaction) are in place from metalake down to the entity level.
2. **Row OCC is not the whole operation.** The remaining external-backed bugs are about *when the identity is captured*: drop, rename and `operateOnEntity` resolve the target by name after the external call, and import trusts a copied `StringIdentifier`. A CAS on the wrong row is still a wrong write. These have to be fixed before the lock goes, because TreeLock hides two of them on a single server.
3. **TreeLock is not only a row lock, but its non-row duties are shrinking.** The catalog lifetime duty (Group A) is already replaced by leases. The plugin-order duty (B1) is the one place where removing the lock changes single-server behaviour with no replacement yet.
4. **Managed metadata is consistent at the row and parent/child level; the relation tables are not yet.** Owner, role membership, and the metadata-object end of tag/policy/statistic relations can still attach to a deleted endpoint or leave two live owners. Fileset create can still overwrite a winner. Manager-level preconditions (non-force drop, enable/disable fan-out) are still outside the deciding transaction.
5. **Some things stay out of reach and are written down as limits**: HMS/Glue whole-object lost updates, exactly-once across the external system and the store, identity continuity of an id-less object that was dropped and re-created under the same name, and physical data that a partially failed fileset delete already removed.
6. So TreeLock removal is the **proposed direction**, not a completed decision: it happens in order, after the identity fixes, the relation protocols and the plugin-order replacement land, and after the two-server tests show that the single-server behaviour did not get worse.

---

## Conclusion

**Choose Direction 1 as the target architecture: do not build a distributed TreeLock around the external-catalog call and the Gravitino-store write.** This is a directional decision, not approval to shrink TreeLock now, and it is still under review in the PR that carries this document.

**TreeLock is removed, but not first.** The review found no concurrency case on external-backed catalogs where a lock is what stands between Gravitino and a bad correctness problem. Concurrent writes there end up either matching, or with a left-over store row that reconcile removes, or with a result the external system itself chose. What the review did find is a small set of **identity** hazards — a delete or an update that resolves its target by name after the external call, an import that trusts a copied id — plus a set of **relation** hazards inside the store, and a small set of **side effects** (Ranger, fileset directories, the job executor) that no row lock covers. These are fixed first. Then the tree comes out, one operation family at a time, each with a mapped invariant and a test that runs without the same-JVM lock.

The gates below must be true before `LockManager` and `TreeLockNode` are touched. The implementation design turns each one into tasks; here they are only stated as conditions, with their status on `main` at 2026-09-15.

| Gate   | Condition                                                                                                                                                                                                                                                                                       | Status                                                                                                                                                                   |
|--------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **G1** | **Operation identity.** No write path can delete or move the attachments of a live entity: the identity observed before the external call flows into a conditional store delete/update; an import never rebinds an id that belongs to another live entity; a `false` external drop leaves the store and the plugin alone | partly: #12232 done, service-level CAS done; dispatcher-level identity capture, import id check, hook-on-`false` open                                                    |
| **G2** | **Managed rows.** Strict create, version-checked update and delete, parent row held in the child's transaction, cascade in one transaction, lifecycle preconditions decided in the same transaction as the delete, from metalake down to the entity level                                      | mostly done; fileset create, manager-level preconditions, metalake in-use fan-out open                                                                                    |
| **G3** | **Relations.** One live owner per object; owner, membership, tag, policy, statistic and securable-object relations cannot attach to a deleted endpoint; assignments are ordered against drop and cascade                                                                                           | tag/policy own side done; owner, membership, object-side fences, statistics open                                                                                          |
| **G4** | **Side effects with identity and recovery.** Catalog wrapper lifetime by lease; authorization-plugin calls carry a version or keep a small guard; fileset storage and job submission have a durable state and idempotent cleanup; a reconcile job removes stale external-backed rows              | lease done; plugin order, fileset state, job intent, reconcile open                                                                                                       |
| **G5** | **Reads.** The read contract is written down: which reads may be served from a stale per-node cache, which must bypass it, and aggregate reads (row + columns, row + versions) either run under one snapshot or re-validate the root version                                                    | eventual invalidation via `EntityChangeLogPoller` exists; the contract is not written                                                                                     |
| **G6** | **Validation.** Every removed lock maps to an invariant and a test; the race tests run against **two** servers sharing one database, on every supported relational backend, driven by barriers rather than timing                                                                               | `TestJobManagerMultiNode` is the only two-node test; service tests cover stale versions and parent fences on H2/MySQL/PostgreSQL                                          |

"Remove TreeLock" therefore does not mean "use no locks." It means removing the JVM-local hierarchical lock from the correctness model. Database statements still take normal row locks, the cross-row protocols deliberately take short parent/endpoint row locks, and a few Java-object lifetimes keep a lease or a small in-process guard. None of those may span an external-catalog RPC — the fileset drop callback, which runs storage deletion inside the store transaction, is a deliberate exception with documented limits, not a template.

---

## Where the implementation lives

This document stops at the decision and the gates. The design of the replacement — the database rules, the identity protocol for external-backed writes, the relation protocols, the side-effect state machines, the read contract, and the staged task list that closes G1–G6 — is in [concurrency-control-implementation-design.md](concurrency-control-implementation-design.md).
...
| What has to be true before the lock is removed?                      | this document (G1–G6) |
...
Seven data-integrity hazards turned up. Two of them exist today with TreeLock in place — a lock never guarded them — so they are independent of the remove-vs-lock decision. They are sub-issues of the epic [#10238](https://github.com/apache/gravitino/issues/10238).

| # | Hazard                                                                                                                                                                 | Level                                     | Exists today with TreeLock?                  | Where it is fixed                                                                                                                                                                                                                | Status on `main` |
|---|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|----------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|
| 1 | id-less catalogs (JDBC, PostgreSQL) have no stable id across a delete, and relations orphaned by out-of-band row loss are never removed                                | 1                                         | **yes**                                      | relation GC ([#12154](https://github.com/apache/gravitino/issues/12154)). Reusing a tombstoned id on re-import was considered and **rejected as unsound** ([#12153](https://github.com/apache/gravitino/issues/12153), closed) | **done** (#12160); membership and owner relation rows still need the same pass ([#13003](https://github.com/apache/gravitino/issues/13003)) |
| 2 | left-over store rows: a failed or swallowed `store.delete`, or a crash, leaves a row with no external object. Only schemas are cleaned today (`OrphanedSchemaCleanup`) | 1, and **2** once the name is reused (S2) | **yes**                                      | reconcile job ([#12155](https://github.com/apache/gravitino/issues/12155)), gate G4                                                                                                                                             | open             |
| 3 | a copied `StringIdentifier` re-binds an existing row and moves its attachments (S1)                                                                                    | **2**                                     | **yes**                                      | import-time id check, gate G1                                                                                                                                                                                                    | open for table/schema/topic; view, function and tag already refuse an id that belongs to another parent (`OccWriteSupport.findAndLockForOverwrite`) |
| 4 | rename against drop, and drop by name, delete a live entity's row and attachments                                                                                       | **2**                                     | hidden on one server                         | [#12232](https://github.com/apache/gravitino/issues/12232) and the version-checked delete, gate G1                                                                                                                              | rename/drop **done**; drop-after-recreate (E1) open |
| 5 | version checks are not really in use: several entities keep the version fixed and compare the whole row instead                                                        | —                                         | yes, but harmless today                      | gate G2                                                                                                                                                                                                                          | **done** ([#12166](https://github.com/apache/gravitino/issues/12166) and its sub-issues) |
| 6 | cross-row relations and state machines were never reviewed: tag, policy, owner, job and statistic operations can race with the deletion of the other end               | 1–2                                       | partly, some are unprotected even in one JVM | gate G3 and the implementation design                                                                                                                                                                                            | tag/policy own side and job done; owner, membership, object side, statistics open |
| 7 | the store write resolves its target by name after the external call, so a re-created object under the same name can be deleted or updated in place of the old one (E1, E2) | **2**                                     | hidden on one server                         | identity capture before the external call, gate G1                                                                                                                                                                               | open             |

One note on hazard 5 and the create paths, checked in the code: catalog, managed table, managed function, model, user, group, role and managed schema insert with `overwrite = false`, so the unique key picks the winner. Only **managed fileset** (`FilesetCatalogOperations`) still uses a blind upsert on create. The remaining `overwrite = true` calls are import and re-registration paths on external-backed dispatchers, where the upsert is intended but, as noted above, does not behave the same on MySQL and PostgreSQL.
