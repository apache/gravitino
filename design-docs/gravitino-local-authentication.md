...
implementation simple while aligning with modern password storage recommendations and avoiding
additional third-party cryptographic dependencies in the server distribution.

To make this implementable, the password hashing design should also define the storage and
dependency model explicitly:

- use the JDK JCA `SecretKeyFactory` implementation for `PBKDF2WithHmacSHA512`
- store the full PBKDF2 hash string in `password_hash`, including algorithm marker, parameters,
...
This keeps verification logic simple and allows future upgrades of PBKDF2 cost parameters without
...
6. If `GRAVITINO_INITIAL_ADMIN_PASSWORD` is configured and the service admin does not yet have a
   password stored in `idp_user_meta`, hash the supplied password with PBKDF2-HMAC-SHA512 and create that
...
The initial implementation uses PBKDF2-HMAC-SHA512 as the fixed password hashing algorithm.
