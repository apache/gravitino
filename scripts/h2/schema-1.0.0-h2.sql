...
CREATE TABLE IF NOT EXISTS `role_meta_securable_object` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'auto increment id',
    `role_id` BIGINT NOT NULL COMMENT 'role id',
    `metadata_object_id` BIGINT NOT NULL COMMENT 'securable object entity id',
    ...
    `current_version` INT NOT NULL DEFAULT 1 COMMENT 'securable objectcurrent version',
...
CREATE TABLE IF NOT EXISTS `policy_version_info` (
    ...
    `enabled` TINYINT DEFAULT 1 COMMENT 'whether the policy is enabled, 0 is disabled, 1 is enabled',
