...
CREATE TABLE IF NOT EXISTS `lance_table_version_info` (
    ...
    `manifest_path` CLOB NOT NULL COMMENT 'manifest path',
    ...
    `metadata_json` CLOB DEFAULT NULL COMMENT 'version metadata',
    `context_json` CLOB DEFAULT NULL COMMENT 'request context',
    ...
    PRIMARY KEY (`id`),
    CONSTRAINT `uk_ltv_tid_ver_del` UNIQUE (`table_id`, `version`, `deleted_at`),
