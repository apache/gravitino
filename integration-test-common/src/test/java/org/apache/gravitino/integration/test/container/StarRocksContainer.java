...
public class StarRocksContainer extends BaseContainer {
  ...
  public static final String DEFAULT_IMAGE = System.getenv("GRAVITINO_CI_STARROCKS_DOCKER_IMAGE");
  ...
  private static final String STARROCKS_BE_PATH = "/opt/apache-starrocks/be/log";
  ...
  public void start() {
    ...
    //    Preconditions.check("StarRocks container password change failed!", changePassword());
  }
  ...
  private void copyStarRocksLog() {
    try {
      // stop StarRocks container
      String destPath = System.getenv("IT_PROJECT_DIR");
      LOG.info("Copy StarRocks log file to {}", destPath);

      String feTarPath = "/starrocks-be.tar";
      String beTarPath = "/starrocks-fe.tar";

      // Pack the jar files
      container.execInContainer("tar", "cf", feTarPath, STARROCKS_BE_PATH);
      container.execInContainer("tar", "cf", beTarPath, STARROCKS_FE_PATH);

      container.copyFileFromContainer(feTarPath, destPath + File.separator + "starrocks-be.tar");
      container.copyFileFromContainer(beTarPath, destPath + File.separator + "starrocks-fe.tar");
    } catch (Exception e) {
  ...
  private boolean changePassword() {
    ...
    try (Connection connection = DriverManager.getConnection(starRocksJdbcUrl, USER_NAME, "");
        Statement statement = connection.createStatement()) {

      String query = String.format("SET PASSWORD FOR '%s' = PASSWORD('%s');", USER_NAME, PASSWORD);
      statement.execute(query);
      LOG.info("StarRocks container password has been changed");
      return true;
    } catch (Exception e) {
      LOG.error(e.getMessage(), e);
    }
