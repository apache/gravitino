...
./build-docker.sh --platform [all|linux/amd64|linux/arm64] --type [gravitino|hive|trino|doris|spark] --image {image_name} --tag {tag_name} --latest
...
elif [ "${component_type}" == "spark" ]; then
  . ${script_dir}/spark/spark-dependency.sh
