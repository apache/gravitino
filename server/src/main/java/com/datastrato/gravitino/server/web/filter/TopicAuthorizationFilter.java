...
/**
 * For TopicAuthorizationFilter, Gravitino only checks whether user is in the metalake.
 * Gravitino allows to handle the privilege issues of the underlying system.
 */
@Provider
@NameBindings.TopicInterfaces
public class TopicAuthorizationFilter extends BaseMetalakeUserFilter {
    @Override
    public void filter(ContainerRequestContext requestContext) throws IOException {
        if (!checkUserInMetalake(requestContext)) {
            requestContext.abortWith(
                    Response.status(
                                    SC_FORBIDDEN, "Only the users in the metalake can execute the topic operations")
                            .build());
        }
    }
}
