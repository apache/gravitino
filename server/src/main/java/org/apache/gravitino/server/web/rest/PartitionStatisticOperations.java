...
@Path("/metalakes/{metalake}/catalogs/{catalog}/schemas/{schema}/tables/{table}/statistics/partitions")
public class PartitionStatisticOperations {
    ...
    public Response listPartitionStatistics(
        ...
        LOG.info("Listing partition statistics for table: {} {}.{}.{} from {} to {}",
                 metalake, catalog, schema, table, fromPartitionName, toPartitionName);
        try {
            return Utils.doAs(httpRequest, () -> {
                MetadataObject metadataObject = MetadataObjects.of(Lists.newArrayList(catalog, schema, table), MetadataObject.Type.TABLE);
                Map<String, List<Statistic>> statistics = partitionStatisticManager.listPartitionStatistics(
                        metalake, metadataObject, fromPartitionName, toPartitionName);
                // Convert DTO

                return Utils.ok(new PartitionStatsListResponse(statistics));

            });
        } catch (Exception e) {
            LOG.error("Error listing partition statistics for table: {} {}.{}.{}",
                    metalake, catalog, schema, table, e);
            return ExceptionHandlers.handlePartitionStatsException(OperationType.LIST, "[" + fromPartitionName + "," + toPartitionName + ")", table, e);
        }
    }
    ...
    public Response updatePartitionStatistics(
        ...
        LOG.info("Updating partition statistics for table: {} {}.{}.{}", metalake, catalog, schema, table);
        try {
            request.validate();
            ...
            return Utils.doAs(httpRequest, () -> {
                ...
                // Add DTOs
                return Utils.ok(new PartitionStatsUpdateResponse());
            });
        } catch (Exception e) {
            LOG.error("Error updating partition statistics for table: {} {}.{}.{}",
                    metalake, catalog, schema, table, e);
            String partitions = StringUtils.joinWith(",", request.getUpdates().keySet());
            return ExceptionHandlers.handlePartitionStatsException(OperationType.UPDATE, partitions, table, e);
        }
    }
    ...
    LOG.info("Dropping partition statistics for table: {} {}.{}.{}", metalake, catalog, schema, table);
        try {
            request.validate();

            return Utils.doAs(httpRequest,
                    () -> Utils.ok(new DropResponse(
                            partitionStatisticManager.dropPartitionStatistics(
                                    metalake,
                                    MetadataObjects.of(Lists.newArrayList(catalog, schema, table), MetadataObject.Type.TABLE),
                                    request.getPartitionStatistics()))));
        } catch (Exception e) {
            LOG.error("Error dropping partition statistics for table: {} {}.{}.{}",
                    metalake, catalog, schema, table, e);
            String partitions = StringUtils.joinWith(",", request.getPartitionStatistics().keySet());
            return ExceptionHandlers.handlePartitionStatsException(OperationType.DROP, partitions, table, e);
        }
    }

    public static class PartitionStatisticManager {
        boolean dropPartitionStatistics(String metalake, MetadataObject metadataObject, Map<String, List<String>> partitionStatistics) {
            return true;
        }

        void updatePartitionStatistics(String metalake, MetadataObject metadataObject, Map<String, Map<String, StatisticValue<?>>> partitionStatistics) {
            return;
        }

        Map<String, List<Statistic>> listPartitionStatistics(String metalake, MetadataObject metadataObject, String fromPartitionName, String toPartitionName) {
            return null;
        }
    }
}
