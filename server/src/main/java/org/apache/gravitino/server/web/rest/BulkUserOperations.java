...
/** REST operations for bulk user mutations. */
@NameBindings.AccessControlInterfaces
@Path("/bulk/metalakes/{metalake}/users")
public class BulkUserOperations {

  private static final Logger LOG = LoggerFactory.getLogger(BulkUserOperations.class);

  private static final String MANAGE_USERS_EXPRESSION = "METALAKE::OWNER || METALAKE::MANAGE_USERS";
  ...
  /** Creates a new BulkUserOperations instance. */
  public BulkUserOperations() {
   ...
   * @param request The bulk user names request.
  ...
  @AuthorizationExpression(expression = MANAGE_USERS_EXPRESSION)
          ...
          () -> {
            ...
            return Utils.ok(
                bulkOperation(
                    ...
                    name -> accessControlManager.addUser(metalake, name).name()));
          });
    } catch (Exception e) {
                ...
                bulkOperation(
                    ...
                    name -> {
                      ...
                      if (!accessControlManager.removeUser(metalake, name)) {
                        LOG.warn("Failed to remove user {} under metalake {}", name, metalake);
  ...
  private void checkMetalakeOwner(String metalake, String user) {
    ownerManager
            ...
            owner -> {
              if (owner.type() == Owner.Type.USER && owner.name().equals(user)) {
                throw new IllegalArgumentException(
                    String.format(
                        "Cannot remove user %s from metalake %s because the user is the owner "
                            + "of the metalake.",
                        user, metalake));
              }
            });
