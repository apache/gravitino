...
public class FunctionOperations {
  ...
  public Response listFunctions(
    ...
    try {
      ...
      return Utils.doAs(
          httpRequest,
          () -> {
            Namespace functionNS = NamespaceUtil.ofFunction(metalake, catalog, schema);
            NameIdentifier[] idents = dispatcher.listFunctions(functionNS);
  ...
  public Response getFunction(
    ...
    LOG.info(
        "Received get function request: {}.{}.{}.{}, version: {}",
        metalake,
        catalog,
        schema,
        function,
    ...
    try {
      return Utils.doAs(
          ...
          () -> {
            ...
            Function f;
            if (version == null) {
              f = dispatcher.getFunction(ident);
            } else {
              Preconditions.checkArgument(
                  version >= 0, "Version must be non-negative, but got:" + version);
              f = dispatcher.getFunction(ident, version);
            }
