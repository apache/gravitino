...
public class TestFunctionOperations extends BaseOperationsTest {
  ...
  public void testListFunctions() {
    ...
    Function mockFunction1 = mockFunction("func1", "comment1", FunctionType.SCALAR, 0);
    Function mockFunction2 = mockFunction("func2", "comment2", FunctionType.SCALAR, 0);
    ...
    Assertions.assertEquals("func1", funcListResp.getFunctions()[0].getName());
    Assertions.assertEquals("func2", funcListResp.getFunctions()[1].getName());
  ...
  public void testGetFunction() {
    Function mockFunction = mockFunction("func1", "test comment", FunctionType.SCALAR, 0);
  ...
  public void testGetFunctionWithVersion() {
    Function mockFunction = mockFunction("func1", "test comment", FunctionType.SCALAR, 1);
    NameIdentifier funcId = NameIdentifierUtil.ofFunction(metalake, catalog, schema, "func1");
    when(functionDispatcher.getFunction(funcId, 1)).thenReturn(mockFunction);
    ...
    FunctionResponse funcResp = resp.readEntity(FunctionResponse.class);
    Assertions.assertEquals(0, funcResp.getCode());
    Assertions.assertEquals(1, funcResp.getFunction().version());

    // Test mock throw NoSuchFunctionVersionException
    doThrow(new NoSuchFunctionVersionException("mock error"))
        .when(functionDispatcher)
        .getFunction(funcId, 1);
    Response resp1 =
        target(functionPath())
            .path("func1")
            .queryParam("version", 1)
    ...
    Assertions.assertEquals(ErrorConstants.NOT_FOUND_CODE, errorResp.getCode());
    Assertions.assertEquals(
        NoSuchFunctionVersionException.class.getSimpleName(), errorResp.getType());

    // Test a negative version
    Response resp2 =
        target(functionPath())
            .path("func1")
            .queryParam("version", -1)
  ...
  public void testRegisterTableFunction() {
    ...
    Function mockFunction = mockTableFunction("tableFunc1", "test comment", 0);
    ...
    FunctionRegisterRequest req =
        FunctionRegisterRequest.builder()
            ...
            .deterministic(true)
            .returnColumns(createMockReturnColumnDTOs())
            .definitions(createMockDefinitionDTOs())
            .build();
  ...
  public void testAlterFunction() {
    NameIdentifier funcId = NameIdentifierUtil.ofFunction(metalake, catalog, schema, "func1");
    Function mockFunction = mockFunction("func1", "new comment", FunctionType.SCALAR, 1);
  ...
  public void testAlterFunctionAddDefinition() {
    NameIdentifier funcId = NameIdentifierUtil.ofFunction(metalake, catalog, schema, "func1");
    Function mockFunction = mockFunction("func1", "comment", FunctionType.SCALAR, 1);
  ...
  private Function mockFunction(
      String name, String comment, FunctionType functionType, int version) {
    ...
    when(mockFunction.version()).thenReturn(version);
    when(mockFunction.auditInfo()).thenReturn(testAuditInfo);
    return mockFunction;
  }

  private Function mockTableFunction(String name, String comment, int version) {
    Function mockFunction = mock(Function.class);
    when(mockFunction.name()).thenReturn(name);
    when(mockFunction.comment()).thenReturn(comment);
