...
public class TestModelOperations extends JerseyTest {
  ...
  @Test
  void testRemoveModelVersionComment() {
    String modelName = "model1";
    String uri = "uri1";
    int version = 0;
    String[] alias = new String[] {"alias1"};
    ...
    NameIdentifier modelId = NameIdentifierUtil.ofModel(metalake, catalog, schema, modelName);
    ModelVersion mockModelVersion = mockModelVersion(version, uri, alias, null);

    when(modelDispatcher.alterModelVersion(modelId, version, ModelVersionChange.removeComment()))
        .thenReturn(mockModelVersion);

    ModelVersionUpdatesRequest req =
        new ModelVersionUpdatesRequest(
            Collections.singletonList(new ModelVersionUpdateRequest.RemoveModelVersionComment()));
    ...
    Assertions.assertNull(modelVersion.comment());
    Assertions.assertArrayEquals(alias, modelVersion.aliases());
    Assertions.assertEquals(version, modelVersion.version());
    Assertions.assertEquals(uri, modelVersion.uri());
  }
  ...
  @Test
  void testRemoveModelVersionCommentByAlias() {
    String modelName = "model1";
    String uri = "uri1";
    int version = 0;
    String[] alias = new String[] {"alias1"};
    ...
    when(modelDispatcher.alterModelVersion(modelId, alias[0], ModelVersionChange.removeComment()))
        .thenReturn(mockModelVersion);
