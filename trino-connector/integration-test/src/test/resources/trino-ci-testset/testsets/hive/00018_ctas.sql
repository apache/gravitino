...
INSERT INTO gt_hive.gt_hive_ctas_db.src_table VALUES (1, 'alice', 10.50), (2, 'bob', 20.75), (3, 'charlie', 30.00);

CREATE TABLE gt_hive.gt_hive_ctas_db.ctas_basic AS SELECT * FROM gt_hive.gt_hive_ctas_db.src_table;

SELECT * FROM gt_hive.gt_hive_ctas_db.ctas_basic ORDER BY id;

-- Test 2: CTAS with column subset and transformation
CREATE TABLE gt_hive.gt_hive_ctas_db.ctas_transform AS SELECT id, upper(name) AS upper_name FROM gt_hive.gt_hive_ctas_db.src_table WHERE id > 1;

SELECT * FROM gt_hive.gt_hive_ctas_db.ctas_transform ORDER BY id;

-- Test 3: CTAS with no data (empty source)
CREATE TABLE gt_hive.gt_hive_ctas_db.ctas_empty AS SELECT * FROM gt_hive.gt_hive_ctas_db.src_table WHERE id < 0;
