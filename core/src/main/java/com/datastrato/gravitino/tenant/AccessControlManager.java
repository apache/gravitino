...
public class AccessControlManager implements SupportsUserManagement {
    ...
    @Override
    public com.datastrato.gravitino.User createUser(String metalake, String userName, Map<String, String> properties) {
        MetalakeUser metalakeUser = MetalakeUser.builder().withId(idGenerator.nextId()).withName(userName).withProperties(properties)
                .withAuditInfo(
                AuditInfo.builder()
        ...
        try {
            ...
            throw new UserAlreadyExistsException("User %s in the metalake %s already exists", userName, metalake);
        } catch (IOException ioe) {
            LOG.error("Creating user {} failed in the metalake {} due to storage issues", userName, metalake, ioe);
    ...
    public boolean dropUser(String metalake, String userName) {
        ...
        try {
            ...
            LOG.error("Deleting user {} in the metalake {} failed due to storage issues", userName, metalake, ioe);
            throw new RuntimeException(ioe);
        }
    }
    ...
    public com.datastrato.gravitino.User loadUser(String metalake, String userName) {
        try {
            return store.get(NameIdentifier.of(metalake, userName), Entity.EntityType.USER, MetalakeUser.class);
        } catch (NoSuchEntityException e) {
            LOG.warn("user {} does not exist in the metalake {}", userName, metalake,  e);
            throw new NoSuchMetalakeException(USER_DOES_NOT_EXIST_MSG, userName, metalake);
        } catch (IOException ioe) {
