 ...
 * a database that supports the JDBC protocol as storage. If the specified
 * database has special SQL
...
public class JDBCBackend implements RelationalBackend {
    ...
    private static final Map<JDBCBackendType, String> EMBEDDED_JDBC_DATABASE_MAP = ImmutableMap.of(JDBCBackendType.H2,
    ...
    public <E extends Entity & HasIdentifier> List<E> listEntitiesByRelation(
        ...
        if (relType != OWNER_REL) {
            throw new IllegalArgumentException(String.format("Doesn't support the relation type %s", relType));
        }
        List<E> list = Lists.newArrayList();
        OwnerMetaService.getInstance().getOwner(nameIdentifier, identType).ifPresent(e -> list.add((E) e));
        return list;
    }
    ...
    /**
     * Start JDBC database if necessary. For example, start the H2 database if the
     * backend is H2.
     */
    private static JDBCDatabase startJDBCDatabaseIfNecessary(Config config) {
        ...
        try {
            JDBCDatabase jdbcDatabase = (JDBCDatabase) Class.forName(EMBEDDED_JDBC_DATABASE_MAP.get(jdbcBackendType))
