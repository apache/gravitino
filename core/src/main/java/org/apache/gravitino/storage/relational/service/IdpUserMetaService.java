...
/** Core facade for built-in IdP user legacy metadata cleanup. */
public class IdpUserMetaService {
  ...
  public int deleteUserMetasByLegacyTimeline(long legacyTimeline, int limit) throws IOException {
    return deleteUserMetasByLegacyTimelineWithCleaner(
        legacyTimeline, limit, ServiceLoader.load(LegacyDataCleaner.class));
  }

  int deleteUserMetasByLegacyTimelineWithCleaner(
      long legacyTimeline, int limit, Iterable<LegacyDataCleaner> cleaners) throws IOException {
    return LegacyDataCleanerLoader.load(Entity.EntityType.IDP_USER, cleaners)
