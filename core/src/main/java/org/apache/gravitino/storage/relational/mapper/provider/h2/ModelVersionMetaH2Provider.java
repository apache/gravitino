...
public class ModelVersionMetaH2Provider extends ModelVersionMetaBaseSQLProvider {
  ...
  public String insertModelVersionMetas(
    ...
    return "<script>"
        ...
        + " SELECT"
        + " CAST(#{version.modelId} AS BIGINT) AS model_id, CAST(#{version.modelVersionComment} AS CLOB) AS model_version_comment,"
        + " CAST(#{version.modelVersionProperties} AS CLOB) AS model_version_properties, CAST(#{version.auditInfo} AS CLOB) AS audit_info,"
  ...
  public String insertModelVersionMetasWithVersionNumber(
    ...
    return "<script>"
        ...
        + " CAST(#{version.modelVersionProperties} AS CLOB) AS model_version_properties, CAST(#{version.auditInfo} AS CLOB) AS audit_info,"
        + " CAST(#{version.deletedAt} AS BIGINT) AS deleted_at, CAST(#{version.modelVersionUriName} AS VARCHAR(128)) AS model_version_uri_name,"
        + " CAST(#{version.modelVersionUri} AS CLOB) AS model_version_uri, CAST(#{version.modelVersion} AS INT) AS model_version_number "
        + "</foreach>"
        + " ) v"
        + " JOIN"
