...
public class FunctionMetaBaseSQLProvider {
  ...
  public String insertFunctionMeta(@Param("functionMeta") FunctionPO functionPO) {
    return "INSERT INTO "
        ...
        + " function_type, `deterministic`, return_type, function_latest_version, audit_info, deleted_at)"
  ...
  public String listFunctionPOsBySchemaId(@Param("schemaId") Long schemaId) {
    return "SELECT fm.function_id, fm.function_name, fm.metalake_id, fm.catalog_id, fm.schema_id,"
        + " fm.function_type, fm.`deterministic`, fm.return_type, fm.function_latest_version,"
        ...
        + " fm INNER JOIN "
        + VERSION_TABLE_NAME
        + " vi ON fm.function_id = vi.function_id AND fm.function_latest_version = vi.version"
        + " WHERE fm.schema_id = #{schemaId} AND fm.deleted_at = 0 AND vi.deleted_at = 0";
  }

  public String selectFunctionMetaWithVersionBySchemaIdAndNameAndVersion(
    ...
    return "SELECT fm.function_id, fm.function_name, fm.metalake_id, fm.catalog_id, fm.schema_id,"
        ...
        + " fm INNER JOIN "
        + VERSION_TABLE_NAME
        + " vi ON fm.function_id = vi.function_id"
        + " AND vi.version = CASE WHEN #{version} = -1 THEN fm.function_latest_version ELSE #{version} END"
  ...
  public String softDeleteFunctionMetaBySchemaIdAndFunctionName(
      @Param("schemaId") Long schemaId, @Param("functionName") String functionName) {
    return "UPDATE "
        + TABLE_NAME
