...
public class TestCaffeineEntityCache {
  ...
  void testGetFromCache() throws IOException {
    ...
    List<? extends Entity> entitiesFromCache =
        cache.getOrLoad(
            ident12, Entity.EntityType.ROLE, SupportsRelationOperations.Type.ROLE_USER_REL, true);
    ...
    Assertions.assertEquals(ImmutableList.of(entity8, entity9), entitiesFromCache);
    ...
    verify(mockStore, never())
        .listEntitiesByRelation(
            SupportsRelationOperations.Type.ROLE_USER_REL, ident12, Entity.EntityType.ROLE, true);
  }
  ...
  void testGetOrLoadWithNull() {
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> cache.getOrLoad(ident12, null, SupportsRelationOperations.Type.ROLE_USER_REL, false));
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> cache.getOrLoad(ident12, Entity.EntityType.ROLE, null, true));
  }
  ...
  private void initCache(Config config) {
    real = CaffeineEntityCache.getInstance(config, mockStore);
    cache = spy(real);
  }

  private void initCache(Config config, EntityStore store) {
    real = CaffeineEntityCache.getInstance(config, store);
    cache = spy(real);
  }
}
