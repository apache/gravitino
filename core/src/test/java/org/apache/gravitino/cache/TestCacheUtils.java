...
public class TestCacheUtils {
  ...
  @Test
  void testDeleteIndexByTableLevel() {
    CacheUtils.deleteByPrefix(indexTree, ident6.toString());

    Assertions.assertEquals(5, indexTree.size());
    Assertions.assertEquals(ident1, indexTree.get(ident1.toString()));
    Assertions.assertEquals(ident2, indexTree.get(ident2.toString()));
    Assertions.assertEquals(ident3, indexTree.get(ident3.toString()));
    Assertions.assertEquals(ident4, indexTree.get(ident4.toString()));
    Assertions.assertEquals(ident5, indexTree.get(ident5.toString()));
  }

  @Test
  void testDeleteIndexBySchemaLevel() {
    CacheUtils.deleteByPrefix(indexTree, ident1.toString());

    Assertions.assertEquals(3, indexTree.size());
    Assertions.assertEquals(ident2, indexTree.get(ident2.toString()));
    Assertions.assertEquals(ident5, indexTree.get(ident5.toString()));
    Assertions.assertEquals(ident6, indexTree.get(ident6.toString()));
  }

  @Test
  void testDeleteIndexByCatalogLevel() {
    CacheUtils.deleteByPrefix(indexTree, ident1.namespace().toString());
  ...
  @Test
  void testDeleteIndexByMetalake() {
    CacheUtils.deleteByPrefix(indexTree, "metalake1");
