...
public class TestEntityStorageRelationCache extends AbstractEntityStorageTest {
  ...
  @ParameterizedTest
  @MethodSource("storageProvider")
  void testUpdateByIdWritesThroughUserCache(String type, boolean enableCache) throws Exception {
    Config config = Mockito.mock(Config.class);
    Mockito.when(config.get(Configs.CACHE_ENABLED)).thenReturn(enableCache);
    init(type, config);
    ...
    try (EntityStore store = EntityStoreFactory.createEntityStore(config)) {
      ...
      Assertions.assertTrue(warmed.enabled());

      store
          .idOperations()
          .updateById(
              AuthorizationUtils.ofUserId("metalake", user.id()),
              ...
              existing ->
                  UserEntity.builder()
                      ...
                      .withExternalId(existing.externalId())
                      .withEnabled(false)
                      .withRoleNames(existing.roleNames())
                      .withRoleIds(existing.roleIds())
                      .withAuditInfo(existing.auditInfo())
                      .build());

      RelationalEntityStore relationalStore = (RelationalEntityStore) store;
      java.util.Optional<UserEntity> cached =
          relationalStore.getCache().getIfPresent(user.nameIdentifier(), Entity.EntityType.USER);
      Assertions.assertTrue(cached.isPresent(), "updateById must write the user back into cache");
      Assertions.assertFalse(cached.get().enabled());

      UserEntity byName =
          store.get(user.nameIdentifier(), Entity.EntityType.USER, UserEntity.class);
      Assertions.assertFalse(byName.enabled());
  ...
  @ParameterizedTest
  @MethodSource("storageProvider")
  void testUpdateWritesThroughUserCache(String type, boolean enableCache) throws Exception {
    Config config = Mockito.mock(Config.class);
    Mockito.when(config.get(Configs.CACHE_ENABLED)).thenReturn(enableCache);
    init(type, config);
    ...
    try (EntityStore store = EntityStoreFactory.createEntityStore(config)) {
      ...
      java.util.Optional<UserEntity> cached =
          relationalStore.getCache().getIfPresent(user.nameIdentifier(), Entity.EntityType.USER);
      Assertions.assertTrue(cached.isPresent(), "update must write the user back into cache");
      Assertions.assertFalse(cached.get().enabled());
  ...
  @ParameterizedTest
  @MethodSource("storageProvider")
  void testUpdateByIdWritesThroughGroupCache(String type, boolean enableCache) throws Exception {
    Config config = Mockito.mock(Config.class);
    Mockito.when(config.get(Configs.CACHE_ENABLED)).thenReturn(enableCache);
    init(type, config);
    ...
    try (EntityStore store = EntityStoreFactory.createEntityStore(config)) {
      ...
      Assertions.assertEquals("ext-old", warmed.externalId());

      store
          .idOperations()
          .updateById(
              AuthorizationUtils.ofGroupId("metalake", group.id()),
      ...
      RelationalEntityStore relationalStore = (RelationalEntityStore) store;
      java.util.Optional<GroupEntity> cached =
          relationalStore.getCache().getIfPresent(group.nameIdentifier(), Entity.EntityType.GROUP);
      Assertions.assertTrue(cached.isPresent(), "updateById must write the group back into cache");
      Assertions.assertEquals("ext-new", cached.get().externalId());

      GroupEntity byName =
          store.get(group.nameIdentifier(), Entity.EntityType.GROUP, GroupEntity.class);
      Assertions.assertEquals("ext-new", byName.externalId());
