...
public class TestEntityStorageRelationCache extends AbstractEntityStorageTest {
  ...
  @ParameterizedTest
  @MethodSource("storageProvider")
  void testMetadataObjectRoleRelationCacheInvalidationOnRoleInsert(String type, boolean enableCache)
      throws Exception {
    Config config = Mockito.mock(Config.class);
    Mockito.when(config.get(Configs.CACHE_ENABLED)).thenReturn(enableCache);
    init(type, config);
    ...
    try (EntityStore store = EntityStoreFactory.createEntityStore(config)) {
      try {
        ...
        SupportsRelationOperations relationOperations = (SupportsRelationOperations) store;
        List<RoleEntity> rolesBeforeInsert =
            relationOperations.listEntitiesByRelation(
                SupportsRelationOperations.Type.METADATA_OBJECT_ROLE_REL,
                function.nameIdentifier(),
                Entity.EntityType.FUNCTION,
                true);
        Assertions.assertTrue(rolesBeforeInsert.isEmpty());
        ...
        List<RoleEntity> rolesAfterInsert =
            relationOperations.listEntitiesByRelation(
                SupportsRelationOperations.Type.METADATA_OBJECT_ROLE_REL,
                function.nameIdentifier(),
                Entity.EntityType.FUNCTION,
                true);
        Assertions.assertEquals(1, rolesAfterInsert.size());
        Assertions.assertEquals(role.name(), rolesAfterInsert.get(0).name());
      } finally {
        destroy(type);
      }
    }
