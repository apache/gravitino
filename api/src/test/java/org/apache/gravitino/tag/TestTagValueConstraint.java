...
public class TestTagValueConstraint {
  ...
  void testNoValueConstraint() {
    ...
    Assertions.assertEquals(TagValueConstraint.Type.NO_VALUE, constraint.type());
    Assertions.assertArrayEquals(new String[0], constraint.allowedValues());
  }
  ...
  void testToStringDifferentiatesConstraintTypes() {
    Assertions.assertEquals(
        "TagValueConstraint{type=ANY_VALUE}", TagValueConstraint.anyValue().toString());
    Assertions.assertEquals(
        "TagValueConstraint{type=NO_VALUE}", TagValueConstraint.noValue().toString());
    Assertions.assertEquals(
        "TagValueConstraint{type=ALLOWED_VALUES, allowedValues=[finance, risk]}",
  ...
  void testEqualsAndHashCode() {
    ...
    Assertions.assertEquals(TagValueConstraint.noValue(), TagValueConstraint.noValue());
