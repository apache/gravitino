...
public class IcebergRESTS3IT extends IcebergRESTJdbcCatalogIT {
  ...
  private void copyS3BundleJar() {
    ...
    String awsBundleFile = String.format("gravitino-aws-bundle-%s.jar", gravitinoVersion);

    String rootDir = System.getenv("GRAVITINO_ROOT_DIR");
    String sourceFile =
        String.format("%s/bundles/aws-bundle/build/libs/%s", rootDir, awsBundleFile);
    String gravitinoHome = System.getenv("GRAVITINO_HOME");
    String targetDir = String.format("%s/iceberg-rest-server/libs/", gravitinoHome);
    String targetFile = String.format("%s/%s", targetDir, awsBundleFile);
