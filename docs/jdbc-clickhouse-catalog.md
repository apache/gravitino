...
- Data-skipping indexes:
  ...
  - `DATA_SKIPPING_TEXT` (native ClickHouse text indexes are recognized when loading a table; `tokenizer` is exposed as `tokens` or `ngrams`, with `ngram_size` from 2 to 8 for `ngrams`; creating or altering text indexes through Gravitino is not supported yet)
