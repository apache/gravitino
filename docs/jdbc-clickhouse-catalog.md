...
| Indexes             | Primary key; data-skipping indexes `DATA_SKIPPING_MINMAX` and `DATA_SKIPPING_BLOOM_FILTER` (customizable `GRANULARITY` via `Index.properties()`, default 1).                                                                                                                                                                                                                                                                                                                |
...
- Data-skipping indexes:
  - `DATA_SKIPPING_MINMAX` (default `GRANULARITY` 1, configurable via `Index.properties()` key `granularity`)
  - `DATA_SKIPPING_BLOOM_FILTER` (default `GRANULARITY` 1, configurable via `Index.properties()` key `granularity`)
  - Note: `ALTER TABLE ADD INDEX` always uses the default `GRANULARITY` because the `TableChange.AddIndex` API has no `properties` field.
