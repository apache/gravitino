...
| Indexes             | Primary key; data-skipping indexes `DATA_SKIPPING_MINMAX`, `DATA_SKIPPING_BLOOM_FILTER`, `DATA_SKIPPING_SET`, `DATA_SKIPPING_NGRAMBFV1`, and `DATA_SKIPPING_TOKENBFV1` (configurable granularity via `Index.properties()`). Legacy `DATA_SKIPPING_ANNOY` and `DATA_SKIPPING_USEARCH` definitions can be read as metadata only.                                                                                                                                                                                 |
...
- Data-skipping indexes:
  ...
  ClickHouse legacy Annoy and USearch indexes with unrepresentable field expressions are also omitted with an explicit warning. The Gravitino index API does not represent arbitrary index expressions.
