...
The Hive catalog stores views as `VIRTUAL_VIEW` in the Hive Metastore. Each view requires exactly
one SQL representation and the `hive` dialect does not accept `defaultCatalog` or `defaultSchema`.
...
-H "Content-Type: application/json" -d '{
  "name": "employee_view",
  "comment": "Active employees",
  "columns": [
    {"name": "id",   "type": "integer", "comment": "employee id",   "nullable": true},
    {"name": "name", "type": "string",  "comment": "employee name", "nullable": true}
  ],
  "representations": [
    {
      "type": "sql",
  ...
  "properties": {}
}' http://localhost:8090/api/metalakes/metalake/catalogs/hive_catalog/schemas/schema/views
```
...
View view = viewCatalog.createView(
    NameIdentifier.of("schema", "employee_view"),
    "Active employees",
    new Column[] {
        Column.of("id",   Types.IntegerType.get(), "employee id"),
        Column.of("name", Types.StringType.get(),  "employee name")
    },
    new Representation[] {
        SQLRepresentation.builder()
            .withDialect("hive")
            .withSql("SELECT id, name FROM employees WHERE active = true")
            .build()
    },
    null,
    null,
...
#### Paimon Catalog Example

The Paimon catalog requires exactly one representation with dialect `query` as the canonical
definition. Additional dialect representations (e.g. `flink`) can be included alongside it.
...
-H "Content-Type: application/json" -d '{
  ...
  "columns": [
    {"name": "id",     "type": "long",   "comment": "order id",     "nullable": true},
    {"name": "amount", "type": "double", "comment": "order amount", "nullable": true}
  ],
  "representations": [
    {
      "type": "sql",
  ...
  "defaultCatalog": "paimon_catalog",
  "defaultSchema": "schema",
  "properties": {}
}' http://localhost:8090/api/metalakes/metalake/catalogs/paimon_catalog/schemas/schema/views
```
...
```java
Catalog catalog = gravitinoClient.loadCatalog("paimon_catalog");
ViewCatalog viewCatalog = catalog.asViewCatalog();

View view = viewCatalog.createView(
    NameIdentifier.of("schema", "order_view"),
    "Completed orders",
    new Column[] {
        Column.of("id",     Types.LongType.get(),   "order id"),
        Column.of("amount", Types.DoubleType.get(), "order amount")
    },
    new Representation[] {
        SQLRepresentation.builder()
            .withDialect("query")
            .withSql("SELECT id, amount FROM orders WHERE status = 'completed'")
            .build()
    },
    "paimon_catalog",
    "schema",
