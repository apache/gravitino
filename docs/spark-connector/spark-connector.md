...
| spark.sql.gravitino.enableIcebergSupport              | string | `false` | Set to `true` to use Iceberg catalog via the Gravitino wrapper catalog.                                                                                                                                                                                          | No       | 0.5.1         |
| spark.sql.gravitino.enablePaimonSupport               | string | `false` | Set to `true` to use Paimon catalog.                                                                                                                                                                                                                             | No       | 1.0.0         |
| spark.sql.gravitino.iceberg.enableRestAccess          | string | `false` | Set to `true` to register each `lakehouse-iceberg` Gravitino catalog as a native Iceberg REST catalog (`org.apache.iceberg.spark.SparkCatalog` with `type=rest`) instead of the Gravitino wrapper catalog. Mutually exclusive with `enableIcebergSupport`.        | No       | 1.0.0         |
| spark.sql.gravitino.iceberg.restUri                   | string | (none)  | Explicit URI for the Gravitino Iceberg REST service (e.g. `http://host:9001/iceberg/`). Used only when `enableRestAccess=true`. When omitted, the URI is inferred from `spark.sql.gravitino.uri` using the default port (HTTP 9001 / HTTPS 9433) and path `/iceberg/`. | No       | 1.0.0         |
