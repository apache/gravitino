...
- [**Metalake operations**](#metalake-operations)
  - [Create a metalake](#create-a-metalake)
  - [Load a metalake](#load-a-metalake)
  - [Alter a metalake](#alter-a-metalake)
  - [Drop a metalake](#drop-a-metalake)
  - [List all metalakes](#list-all-metalakes)
- [**Catalogs operations**](#catalogs-operations)
  - [Create a catalog](#create-a-catalog)
  - [Load a catalog](#load-a-catalog)
  - [Alter a catalog](#alter-a-catalog)
  - [Drop a catalog](#drop-a-catalog)
  - [List all catalogs in a metalake](#list-all-catalogs-in-a-metalake)
- [**Schemas operations**](#schemas-operations)
  - [Create a schema](#create-a-schema)
  - [Load a schema](#load-a-schema)
  - [Alter a schema](#alter-a-schema)
  - [Drop a schema](#drop-a-schema)
  - [List all schemas under a catalog](#list-all-schemas-under-a-catalog)
- [**Tables operations**](#tables-operations)
  - [Create a table](#create-a-table)
  - [Load a table](#load-a-table)
  - [Alter a table](#alter-a-table)
  - [Drop a table](#drop-a-table)
  - [List all tables under a schema](#list-all-tables-under-a-schema)

In this document, Gravitino would use Apache Hive catalog as an example to show how to manage Gravitino metadata. Other Catalogs are similar to Hive catalog.

Assuming Gravitino has just started, and the host and post is `http://localhost:8090`.

## Metalake operations

### Create a metalake

Users can create a metalake by sending a `POST` request to the `/api/metalakes` endpoint or just use the Gravitino Java client.
...
Users can create a metalake by sending a `GET` request to the `/api/metalakes/{metalake_name}` endpoint or just use the Gravitino Java client. The following is an example of loading a metalake:
...
Users can modify a metalake by sending a `PUT` request to the `/api/metalakes/{metalake_name}` endpoint or just use the Gravitino Java client. The following is an example of altering a metalake:
...
| Add a property         | `{"@type":"setProperty","property":"key1","value":"value1"}` | `MetalakeChange.setProperty("key1", "value1")`  |
| Remove a property      | `{"@type":"removeProperty","property":"key1"}`               | `MetalakeChange.removeProperty("key1")`         |


### Drop a metalake

Users can remove a metalake by sending a `DELETE` request to the `/api/metalakes/{metalake_name}` endpoint or just use the Gravitino Java client. The following is an example of dropping a metalake:
...
:::note
Drop a metalake will only remove metadata about the metalake and catalogs, schemas, tables under the metalake in Gravitino, It will not remove the real data(table and schema) in Apache Hive.
::::

### List all metalakes

Users can list metalakes by sending a `GET` request to the `/api/metalakes` endpoint or just use the Gravitino Java client. The following is an example of listing all metalake name:
...
Users can create a catalog by sending a `POST` request to the `/api/metalakes/{metalake_name}/catalogs` endpoint or just use the Gravitino Java client. The following is an example of creating a catalog:
...
Users can load a catalog by sending a `GET` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}` endpoint or just use the Gravitino Java client. The following is an example of loading a catalog:
...
Users can modify a catalog by sending a `PUT` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}` endpoint or just use the Gravitino Java client. The following is an example of altering a catalog:
...
| Add a property         | `{"@type":"setProperty","property":"key1","value":"value1"}` | `CatalogChange.setProperty("key1", "value1")`  |
| Remove a property      | `{"@type":"removeProperty","property":"key1"}`               | `CatalogChange.removeProperty("key1")`         |


### Drop a catalog

Users can remove a catalog by sending a `DELETE` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}` endpoint or just use the Gravitino Java client. The following is an example of dropping a catalog:
...
:::note
Drop a catalog will only remove metadata about the catalog and schemas, tables under the catalog in Gravitino, It will not remove the real data(table and schema) in Apache Hive.
:::

### List all catalogs in a metalake

Users can list all catalogs under a metalake by sending a `GET` request to the `/api/metalakes/{metalake_name}/catalogs` endpoint or just use the Gravitino Java client. The following is an example of listing all catalogs in
a metalake:
...
Users can create a schema by sending a `POST` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas` endpoint or just use the Gravitino Java client. The following is an example of creating a schema:
...
Users can create a schema by sending a `GET` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}` endpoint or just use the Gravitino Java client. The following is an example of loading a schema:
...
### Alter a schema
Users can change a schema by sending a `PUT` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}` endpoint or just use the Gravitino Java client. The following is an example of modifying a schema:
...
| Supported modification | Bash                                                         | Java                                           |
|------------------------|--------------------------------------------------------------|------------------------------------------------|
| Add a property         | `{"@type":"setProperty","property":"key1","value":"value1"}` | `SchemaChange.setProperty("key1", "value1")`  |
| Remove a property      | `{"@type":"removeProperty","property":"key1"}`               | `SchemaChange.removeProperty("key1")`         |


### Drop a schema
Users can remove a schema by sending a `DELETE` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}` endpoint or just use the Gravitino Java client. The following is an example of droping a schema:
...
Users can alter all schemas under a catalog by sending a `GET` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas` endpoint or just use the Gravitino Java client. The following is an example of list all schema
    in a catalog:
...
Users can create a table by sending a `POST` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}/tables` endpoint or just use the Gravitino Java client. The following is an example of creating a table:
...
:::note
The following features aren't supported by all catalogs. Please refer to the related doc for more details.
:::
...
Users can load a table by sending a `GET` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}/tables/{table_name}` endpoint or just use the Gravitino Java client. The following is an example of loading a table:
...
Users can modify a table by sending a `PUT` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}/tables/{table_name}` endpoint or just use the Gravitino Java client. The following is an example of modifying a table:
...
| Supported modification             | Bash                                                                                                                  | Java                                        |
|------------------------------------|-----------------------------------------------------------------------------------------------------------------------|---------------------------------------------|
| Rename metalake                    | `{"@type":"rename","newName":"table_renamed"}`                                                                        | `TableChange.rename("table_renamed")`       |
| Update comment                     | `{"@type":"updateComment","newComment":"new_comment"}`                                                                | `TableChange.updateComment("new_comment")`  |
| Add a table property               | `{"@type":"setProperty","property":"key1","value":"value1"}`                                                          | `TableChange.setProperty("key1", "value1")` |
| Remove a column                    | `{"@type":"removeProperty","property":"key1"}`                                                                        | `TableChange.removeProperty("key1")`        |
...
Users can remove a table by sending a `DELETE` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}/tables/{table_name}` endpoint or just use the Gravitino Java client. The following is an example of droping a table:
...
Users can list all tables in a schema by sending a `GET` request to the `/api/metalakes/{metalake_name}/catalogs/{catalog_name}/schemas/{schema_name}/tables` endpoint or just use the Gravitino Java client. The following is an example of list all tables in a schema:
