...
The mockups use a minimal metric set by default: Decision Metrics determine whether maintenance should run, and Validation Metrics verify the completed job with before/after table metrics. Query speed metrics are optional when query history is available.
...
- Trigger thresholds:
  - Minimum small file count or ratio.
  - Minimum delete file count.
  - Target file size.
  - Advanced: data file MSE, shown only when users expand the generated rule details.
- Ranking weights:
  - Small file weight.
  - Delete file weight.
  - File size target weight.
  - Cost weight.
- Job generation:
...
Cost represents estimated maintenance effort, not cloud billing. The first version should keep the
cost model intentionally small so the UI can work with data already produced by table metrics.

Recommended first-version formula:

```text
estimatedRuntimeMinutes = baseOverheadMinutes
  + inputFileCount * fileOverheadMinutes
  + inputSizeGiB * rewriteMinutesPerGiB

costScore = normalize(estimatedRuntimeMinutes)
```

Required inputs:

- `inputFileCount`: files selected for maintenance.
- `inputSizeGiB`: selected data size in GiB.

Optional inputs:

- `selectedPartitionCount`: shown only when partition-level metrics are available.
- `recentFailureCount`: shown only in job history and not required for the first cost score.

Initial configurable weights:

- `baseOverheadMinutes`: fixed startup and planning cost.
- `fileOverheadMinutes`: per-file planning and open overhead.
- `rewriteMinutesPerGiB`: rewrite cost per GiB.

The UI should show:

- Estimated runtime: for example `18 min`.
- Cost level: Low, Medium, or High.
- Cost breakdown: file count and data size.

### Benefit

Benefit represents expected query-speed improvement, but the first version should estimate it from
a small table-metric proxy. Query history can improve confidence, but it is not required.

Required first-version benefit inputs:

- `fileCountBefore` and `estimatedFileCountAfter`.
- `smallFileCountBefore` and `estimatedSmallFileCountAfter`.
- `averageFileSizeBefore` and `targetFileSize`.
- `deleteFileCountBefore` and `estimatedDeleteFileCountAfter`.

Recommended first-version formula:

```text
fileReductionRatio = (fileCountBefore - estimatedFileCountAfter) / fileCountBefore
smallFileReductionRatio =
  (smallFileCountBefore - estimatedSmallFileCountAfter) / smallFileCountBefore
deleteFileReductionRatio =
  (deleteFileCountBefore - estimatedDeleteFileCountAfter) / max(deleteFileCountBefore, 1)
fileSizeImprovement = closeness(averageFileSizeBefore, targetFileSize)

benefitScore = normalize(
  fileReductionRatio * fileReductionWeight
  + smallFileReductionRatio * smallFileWeight
  + deleteFileReductionRatio * deleteFileWeight
  + fileSizeImprovement * fileSizeWeight
)
```

The UI should show benefit in simple terms:

- Estimated file reduction.
- Estimated small file reduction.
- Estimated delete file reduction.
- Average file size moving toward target size.
- Benefit level: Low, Medium, or High.

Optional query-history validation:

- If query history is available, compare `p95` query duration before and after maintenance.
- If query counts are available, also show query minutes saved.
- If query history is unavailable, label the result as table-metric validation rather than observed
  query speedup.

Post-job validation should use the same small metric set by default:

- File count before and after.
- Small file count before and after.
- Average file size before and after.
- Delete file count before and after.
- Optional `p95` query duration before and after.
