...
| `delete-data-on-drop`                 | Whether to delete the storage locations of a managed fileset when it is dropped. Set to `false` to keep the data.        | true                                                                                                           | No                                         | No        |
| `allow-existing-location-as-managed`  | Whether a managed fileset may be created on a location that already exists and is not empty. Such a location is rejected by default; register it as an `EXTERNAL` fileset instead. | false                                                                       | No                                         | Yes       |
