...
|----------------------------------------------------------|--------|---------------|---------------------------------------------------------------|----------|
| table.catalog-store.gravitino.gravitino.client.auth.type | string | (none)        | When explicitly set, only `oauth2` and `basic` are supported. | No       |
...
| Property                                                         | Type   | Default Value | Description                                      | Required             |
|------------------------------------------------------------------|--------|---------------|--------------------------------------------------|----------------------|
| table.catalog-store.gravitino.gravitino.client.oauth2.serverUri  | string | (none)        | The OAuth2 server URI.                           | Yes, for OAuth2 mode |
| table.catalog-store.gravitino.gravitino.client.oauth2.tokenPath  | string | (none)        | The token endpoint path on the OAuth2 server.    | Yes, for OAuth2 mode |
| table.catalog-store.gravitino.gravitino.client.oauth2.credential | string | (none)        | The credential used to request the OAuth2 token. | Yes, for OAuth2 mode |
| table.catalog-store.gravitino.gravitino.client.oauth2.scope      | string | (none)        | The scope used to request the OAuth2 token.      | Yes, for OAuth2 mode |
