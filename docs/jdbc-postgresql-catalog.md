...
When you use the Gravitino in Trino. You can pass the Trino PostgreSQL connector configuration using prefix `trino.bypass`. For example using `trino.bypass.join-pushdown.strategy` to passing the `join-pushdown.strategy` to the Gravitino PostgreSQL catalog in Trino runtime.
