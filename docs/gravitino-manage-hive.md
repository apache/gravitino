...
Gravitino offers the capability to utilize Apache Hive as a catalog for metadata management. This guide leads you through the process of creating a catalog using Hive in Gravitino.
...
* `name`: The name of the created Hive catalog.
* `comment`:  Optional, user custom catalog comment.
* `provider`: Must set this to "hive" to use Hive as the catalog provider.
* `type`: Must set this to "RELATIONAL" because Hive has a relational data structure, like `db.table`.
* `properties`: The properties of the Hive catalog. For more property information see the following catalog properties table.
...
| `gravitino.bypass.` | Property name with this prefix passed down to the underlying HMS client for use. Empty by default value.             | `gravitino.bypass.hive.metastore.failure.retries = 3` indicate 3 times of retries upon failure of Thrift metastore calls | 0.2.0         |

## Creating an Apache Hive schema

After creating the catalog, you can submit a schema JSON example to the Gravitino server using the URL format:
...
* `name`: The name of the created Hive database.
* `comment`: Optional, user custom Hive database comment.
* `properties`: The properties of the Hive database. For more property information see the following schema properties table. You can pass down other properties to the underlying Hive database parameters.
...
* `name`: The name of the created Hive table.
* `comment`: Optional, user custom Hive table comment.
* `columns`: The columns of the Hive table.
* `partitions`: Optional, the partitions of the Hive table, the previous example is a partitioned table with `dt` column.
* `distribution`: Optional, equivalent to the `CLUSTERED BY` clause in Hive DDL, the previous example table bucketed or cluster on the `id` column.
* `sortOrders`: Optional, equivalent to the `SORTED BY` clause in Hive DDL, the previous example table shows shorted data in increasing order of `age` in each bucket.
* `properties`: The properties of the Hive table. For more property information see the following table properties table. You can pass down other properties to the underlying Hive table parameters.
