...
- When loading an existing HMS view, Gravitino automatically detects whether the view uses the `hive`, `flink`, `spark`, or `trino` dialect. A `trino` view (created outside Gravitino as a Presto/Trino view) can be loaded but not created, since Gravitino doesn't support creating views with the `trino` dialect yet.
