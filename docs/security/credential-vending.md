...
Sensitive catalog properties such as `s3-access-key-id`, `s3-secret-access-key`, `jdbc-user`, and `jdbc-password` are excluded from the default `GET /api/metalakes/{metalake}/catalogs/{catalog}` response. Use the credentials API (`getCredentials` / `JdbcCredential`) for credential-vending and JDBC identity. Other secret-manager-backed properties can be retrieved via `getSecretProperties` / `GET .../objects/{type}/{fullName}/secret-properties`. Clients written against earlier releases that read those properties directly from the default load lose access to them.
