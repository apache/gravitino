...
| `gravitino.idp.basic.verifiedCredentialCacheExpirationSecs` | How long a successfully verified Basic credential may skip password re-derivation. `0` disables the cache. Failed logins are never cached. Password changes and disables take effect on the next request because authentication always reloads the user from storage. | `60` |
| `gravitino.idp.basic.verifiedCredentialCacheMaxSize` | Maximum number of successfully verified Basic credentials retained in memory | `10000` |
