...
For the complete list of authentication properties (general, OAuth, Kerberos, signature algorithms), see [Server Configuration > Authentication](../gravitino-server-config.md#authentication).

Gravitino server and the embedded Gravitino Iceberg REST server share the same authentication properties; you do not need to add a `gravitino.iceberg-rest` prefix for the Iceberg REST server.
