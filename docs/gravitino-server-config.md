...
| `gravitino.entity.store.relational.maxConnections`| The maximum number of connections for the JDBC Backend **write** pool. When a read replica is configured, the read pool uses `gravitino.entity.store.relational.readOnlyMaxConnections` or this value when that is `-1`.                                  | `100`                             | No                                              | 0.9.0-incubating |
| `gravitino.entity.store.relational.maxWaitMillis` | The maximum wait time in milliseconds for a connection from the JDBC Backend **write** pool. When a read replica is configured, the read pool uses `gravitino.entity.store.relational.readOnlyMaxWaitMillis` or this value when that is `-1`.           | `1000`                            | No                                              | 0.9.0-incubating |
| `gravitino.entity.store.relational.jdbcReadOnlyUrl` | Optional JDBC URL for read-only traffic (e.g. a MySQL replica). When set (with same DB kind as primary), standalone metadata reads use this pool; writes and in-transaction reads use the primary. If unset or blank, all traffic uses the primary URL.   | (none)                            | No                                              | 1.3.0            |
| `gravitino.entity.store.relational.jdbcReadOnlyUser` | Optional username for the read-only connection. If unset or blank, the primary `jdbcUser` is used.                                                                                                                                                       | (none)                            | No                                              | 1.3.0            |
| `gravitino.entity.store.relational.jdbcReadOnlyPassword` | Optional password for the read-only connection. If unset or blank, the primary `jdbcPassword` is used.                                                                                                                                                  | (none)                            | No                                              | 1.3.0            |
| `gravitino.entity.store.relational.readOnlyMaxConnections` | Maximum connections for the read-only pool. Use `-1` to use the same value as `maxConnections`. Only applies when a read replica is enabled.                                                                                                            | `-1`                              | No                                              | 1.3.0            |
| `gravitino.entity.store.relational.readOnlyMaxWaitMillis` | Maximum wait time in milliseconds for the read-only pool. Use `-1` to use the same value as `maxWaitMillis`. Only applies when a read replica is enabled.                                                                                               | `-1`                              | No                                              | 1.3.0            |

#### Read replica (optional)

When you set `gravitino.entity.store.relational.jdbcReadOnlyUrl` (or any of the other read-only options), Gravitino creates a second JDBC pool and routes **standalone** metadata reads to it. Writes and all operations inside an open transaction use the primary pool.

:::caution
**Consistency and routing caveats**

- **Replica lag**: Standalone reads can see replica lag (eventual consistency). If a client writes and then immediately reads in a separate request, the read may not yet reflect the write.
- **Strong consistency**: If you need read-your-writes consistency across requests, use synchronous replication on the database side. **MySQL semi-sync** guarantees that at least one replica has received the transaction after commit; it does **not** guarantee that every replica has it. If your read URL points to multiple replicas (e.g. behind a load balancer), a read can hit a replica that has not received the write yet. For read-your-writes with multiple replicas, use **MySQL Group Replication** (or equivalent) so that all members of the group commit together. With a **single** read replica, semi-sync can be sufficient.
- **Routing by API, not SQL**: Read vs write is determined by whether the operation is performed via a commit/transaction API (write) or a standalone read-only API (read). Gravitino does not analyze SQL. Misuse (e.g. issuing DML via the read path at top level) could in theory hit the read pool; in normal use, such calls run under an active write transaction and stay on the primary.
:::
...
| policy operation                        | `CreatePolicyEvent`, `AlterPolicyEvent`, `DeletePolicyEvent`, `GetPolicyEvent`, `ListPoliciesEvent`, `AssociatePoliciesForMetadataObjectEvent`, `ListAssociatedPoliciesForMetadataObjectEvent`, `CreatePolicyFailureEvent`, `AlterPolicyFailureEvent`, `DeletePolicyFailureEvent`, `GetPolicyFailureEvent`, `ListPoliciesFailureEvent`, `AssociatePoliciesForMetadataObjectFailureEvent`, `ListAssociatedPoliciesForMetadataObjectFailureEvent` | 1.1.0            |
...
| policy operation                        | `CreatePolicyPreEvent`, `AlterPolicyPreEvent`, `DeletePolicyPreEvent`, `GetPolicyPreEvent`, `ListPoliciesPreEvent`, `AssociatePoliciesForMetadataObjectPreEvent`, `ListAssociatedPoliciesForMetadataObjectPreEvent`                                                                                                                | 1.1.0 |
