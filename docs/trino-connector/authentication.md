...
| `gravitino.trino.session-cache.max-size`                   | Maximum number of per-user sessions to keep in the cache                                   | `500`         | No       | 1.3.0         |
| `gravitino.trino.session-cache.expire-after-access-seconds`| Seconds before an idle per-user session is evicted from the cache                          | `3600`        | No       | 1.3.0         |
