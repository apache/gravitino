...
Here `gvfs` is the scheme of the GVFS, `fileset` is the root directory of the GVFS which can't modified,
and `${catalog_name}/${schema_name}/${fileset_name}` is the virtual path of the fileset.
You can access the files and folders under this virtual path by concatenating a file or folder name to the virtual path.
...
Apart from the above properties, to access fileset like S3, GCS, OSS and custom fileset, extra properties are needed.
...
[OSS GVFS Java client configurations](./catalogs/fileset/hadoop/oss.md#using-the-gvfs-java-client-to-access-the-fileset) and
...
1. Configure the properties in the `core-site.xml` file of the Hadoop environment:
...
1. Compile from the source code:
   ...
   Download or clone the [Gravitino source code](https://github.com/apache/gravitino),
   and compile it locally using the following command in the Gravitino source code directory:
...
1. Add the GVFS runtime jar to the Spark environment.
   ...
   add it to the `${SPARK_HOME}/jars/` folder.
...
1. Perform operations on the fileset storage in your code.
...
1. First, add a patch and recompile tensorflow-io.
   ...
   Then you can follow the [tutorial](https://github.com/tensorflow/io/blob/master/docs/development.md)
   to recompile your code and release the tensorflow-io module.

1. Then you need to configure the Hadoop configuration.
...
please refer to this document to complete the configuration of the Gravitino server and the OAuth server:
...
please refer to this document to complete the configuration of the Gravitino server:
[Security](./security/security.md).
...
[OSS GVFS Java client configurations](./catalogs/fileset/hadoop/oss.md#using-the-gvfs-python-client-to-access-a-fileset) and
...
:::note
Gravitino python client does not support [customized file systems](./catalogs/fileset/hadoop/hadoop-catalog.md#how-to-custom-your-own-hcfs-file-system-fileset)
...
You can also perform operations on the files or directories managed
by fileset integrating with some Third-party Python libraries which support fsspec compatible filesystems.
...
Currently, Gravitino Virtual File System in Python supports two kinds of authentication types to access Gravitino server:
`simple` and `oauth2`.
