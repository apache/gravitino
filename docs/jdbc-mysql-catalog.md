...
When you use the Gravitino in Trino. You can pass the Trino MySQL connector configuration using prefix `trino.bypass`. For example using `trino.bypass.join-pushdown.strategy` to passing the `join-pushdown.strategy` to the Gravitino MySQL catalog in Trino runtime.
