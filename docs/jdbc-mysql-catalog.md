...
- doesn't support setting certain column properties, such as check constraints.
- supports index definition.
- supports [column default value](./manage-metadata-using-gravitino.md#table-column-default-value) and [auto-increment](./manage-metadata-using-gravitino.md#table-column-auto-increment).
- supports managing MySQL table features though table properties, e.g. using `engine` to set the MySQL storage engine.
...
Any data types not listed above are mapped to Gravitino **[Unparsed Type](./manage-metadata-using-gravitino.md#unparsed-type)** that represents an unresolvable data type since Gravitino 0.5.0.
:::
