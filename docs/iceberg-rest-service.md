...
| `gravitino.iceberg-rest.advertised-uri`          | The public endpoint reported to clients that discover the Iceberg REST service through the Gravitino server, such as the Trino connector. Set it when clients reach the service through a reverse proxy whose scheme, host, port or path differs from the listener's, for example `https://iceberg.example.com/iceberg/`. Must be an absolute `http` or `https` URI, otherwise discovery requests fail; it only affects the advertised endpoint, not the listener. When unset, the endpoint is derived from `host`, `httpPort`/`httpsPort` and `enableHttps`. Auxiliary service only. | (none)                                                                       | No       |
