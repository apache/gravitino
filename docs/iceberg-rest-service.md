...
| `gravitino.iceberg-rest.advertised-uri`          | The public endpoint reported to clients that discover the service through the Gravitino server, such as the Trino connector. See the note below.                                              | (none)                                                                       | No       |
...
Set `gravitino.iceberg-rest.advertised-uri` when clients reach the service through a reverse proxy whose scheme, host, port or path differs from the listener's, for example `https://iceberg.example.com/iceberg/`.
It must be an absolute `http` or `https` URI, otherwise discovery requests fail.
