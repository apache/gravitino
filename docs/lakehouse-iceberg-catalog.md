...
| `format-version`          | The Iceberg table format version. Valid values are `2` and `3`; version `3` is required for V3 types such as `variant`. Gravitino defaults it to `2` when unset. | `2`           | No       | No       | Yes       | 0.2.0         |
