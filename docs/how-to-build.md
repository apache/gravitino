...
For installing WSL, refer to the [WSL Installation Guide](https://learn.microsoft.com/en-us/windows/wsl/install).
...
After installing the JDK, you may want to setup your Shell environment ensure that the correct JDK version will be used.
...
- macOS uses [docker-connector](https://github.com/wenjunxiao/mac-docker-connector)
  ...
  To avoid vendor-related issues on some platforms, for example,
  Gravitino uses the specified Amazon Corretto OpenJDK 17 to build the Trino connector on macOS.
...
1. Build the Gravitino project. Running this for the first time can take 15 minutes or more.
   ...
   You can use JRE 8, 11 or 17 to run the Gravitino server,
   no matter which JDK version was used to build the project.
 
   The built JARs are under the modules `build/libs` directory.
   You can publish them in the Maven repositor for your projecty
   :::
...
1. Build the Gravitino server binary package.
   ...
   You can use the `-x test` flag to explicitly skip tests.
