 ...
 * configurable behavior for quiet mode, output limiting, and sorting.
 *
 * @author pancx
 */
public abstract class BaseOutputFormat<T> implements OutputFormat<T> {
   ...
   *
   * @param quiet If true, suppresses all output except for errors.
   * @param limit Maximum number of items to output (use a negative for unlimited).
   * @param sort If true, output will be sorted according to implementation.
   */
  public BaseOutputFormat(boolean quiet, int limit, boolean sort) {
    this.quiet = quiet;
    this.limit = limit;
    this.sort = sort;
  }
