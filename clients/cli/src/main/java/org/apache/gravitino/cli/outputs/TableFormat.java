...
public abstract class TableFormat<T> extends BaseOutputFormat<T> {
   ...
   * @param property Output configuration properties.
   * @throws IllegalArgumentException if the object type is not supported
   */
  public static void output(Object entity, OutputProperty property) {
    if (entity instanceof Metalake) {
      new MetalakeTableFormat(property).output((Metalake) entity);
    } else if (entity instanceof Metalake[]) {
      new MetalakesTableFormat(property).output((Metalake[]) entity);
    } else if (entity instanceof Catalog) {
      new CatalogTableFormat(property).output((Catalog) entity);
    } else if (entity instanceof Catalog[]) {
      new CatalogsTableFormat(property).output((Catalog[]) entity);
    } else if (entity instanceof Schema) {
      new SchemaTableFormat(property).output((Schema) entity);
    } else if (entity instanceof Schema[]) {
      new SchemasTableFormat(property).output((Schema[]) entity);
    } else {
   ...
   *
   * @param property Configuration for table formatting and output behavior
   */
  ...
  public String getTableFormat(Column... columns) {
    ...
    if (isRowNumbersEnabled()) {
      Column numberColumn = new Column("", "", property);
      int cellCount = columns[0].getCellCount();
      for (int i = 0; i < cellCount; i++) {
        numberColumn.addCell(String.valueOf(i + 1));
      }
      columns = LineUtil.addFirst(numberColumn, columns);
    }
  ...
  private void checkColumns(Column... columns) {
    // TODO: automatic filling of missing data with empty strings
  ...
  /**
   * Sort the columns based on the first column.
   *
   * @param columns the array of columns to sort.
   * @return the sorted array of columns.
   */
  private Column[] getSortedColumns(Column[] columns) {
  ...
  /**
   * Writes the footer row of the table with proper alignment and borders. Processes all column
   * footers, filters out null values, and applies footer-specific alignments for each column.
   *
   * @param osw The output stream writer to write the formatted footer.
   * @param borders List of border characters where: borders.get(4) - left border character.
   *     borders.get(5) - column separator character. borders.get(6) - right border character.
   * @param columns Array of columns containing footer content and formatting properties.
   * @throws IOException If an error occurs while writing to the output stream.
   */
  private static void writeFooter(OutputStreamWriter osw, List<Character> borders, Column[] columns)
      throws IOException {
    String[] footers =
        Arrays.stream(columns)
            .map(Column::getFooter)
  ...
  private static void writeJustified(
      OutputStreamWriter osw, String str, HorizontalAlign align, int maxLength, int minPadding)
      throws IOException {
  ...
  static final class MetalakeTableFormat extends TableFormat<Metalake> {
    ...
    public MetalakeTableFormat(OutputProperty property) {
      super(property);
    }
  ...
  static final class MetalakesTableFormat extends TableFormat<Metalake[]> {
    ...
    public MetalakesTableFormat(OutputProperty property) {
      super(property);
    }
  ...
  static final class CatalogTableFormat extends TableFormat<Catalog> {
    ...
    public CatalogTableFormat(OutputProperty property) {
      super(property);
    }
    ...
    public String getOutput(Catalog catalog) {
      ...
      Column columnB = new Column("TYPE", null, property);
      Column columnC = new Column("provider", null, property);
      Column columnD = new Column("comment", null, property);
  ...
  static final class CatalogsTableFormat extends TableFormat<Catalog[]> {
    ...
    public CatalogsTableFormat(OutputProperty property) {
      super(property);
    }
  ...
  static final class SchemaTableFormat extends TableFormat<Schema> {
    public SchemaTableFormat(OutputProperty property) {
      super(property);
    }
    ...
    public String getOutput(Schema schema) {
      Column columnA = new Column("SCHEMA", null, property);
      Column columnB = new Column("COMMENT", null, property);
  ...
  static final class SchemasTableFormat extends TableFormat<Schema[]> {
    public SchemasTableFormat(OutputProperty property) {
      super(property);
    }
