...
class MetadataObjectCredentialOperations(SupportsCredentials):
  def __init__(self, metalake_name: str, metadata_object: MetadataObject,
      rest_client: HTTPClient):
    self._rest_client = rest_client
    t = metadata_object.type().value
    name = metadata_object.name()
    self._request_path = f"api/metalakes/{metalake_name}/objects/{t}/{name}/credentials"
    pass
  ...
  def to_credentials(self, credentials: List[CredentialDTO]) -> List[
    Credential]:
  ...
  def to_credential(self, credentialDTO: CredentialDTO) -> Credential:
    return CredentialUtils.to_credential(credentialDTO.credential_type(), credentialDTO.credential_info(), credentialDTO.expire_time_in_ms())
