...
class GenericFileset(Fileset, SupportsCredentials):
  ...
  def __init__(self, fileset: FilesetDTO, rest_client: HTTPClient, full_namespace: Namespace):
    self._fileset = fileset
    metadata_object = MetadataObjectImpl([full_namespace.level(1), full_namespace.level(2), fileset.name()], MetadataObject.Type.FILESET)
    self._credential_operations = MetadataObjectCredentialOperations(full_namespace.level(0), metadata_object, rest_client)
