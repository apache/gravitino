...
class TestResponses(unittest.TestCase):
  ...
  def test_credential_response(self):
    ...
    json_str = json.dumps(json_data)
    credential_resp: CredentialResponse  = CredentialResponse.from_json(
      json_str
    )
    self.assertEqual(0, len(credential_resp.credentials()))
    credential_resp.validate()

    json_data = {"code":0,"credentials":[{"credentialType":"s3-token","expireTimeInMs":1000,"credentialInfo":{"s3-access-key-id":"access-id","s3-secret-access-key":"secret-key","s3-session-token":"token"}}]}
    json_str = json.dumps(json_data)
    credential_resp: CredentialResponse  = CredentialResponse.from_json(
      json_str
    )
    ...
    self.assertEqual("secret-key", credential.credential_info()["s3-secret-access-key"])
