...
class TestCredentialUtils(unittest.TestCase):
  ...
  def test_gcs_token_credential(self):
    ...
    self.assertEqual(GCSTokenCredential.GCS_TOKEN_CREDENTIAL_TYPE,
                     check_credential.credential_type())
    self.assertEqual("token", check_credential.token())

  def test_oss_token_credential(self):
    credential_info = {OSSTokenCredential._GRAVITINO_OSS_TOKEN: "token",
    ...
    check_credential = CredentialUtils.to_credential(
      credential.credential_type(), credential_info, expire_time)
    self.assertEqual(OSSTokenCredential.OSS_TOKEN_CREDENTIAL_TYPE,
                     check_credential.credential_type())
