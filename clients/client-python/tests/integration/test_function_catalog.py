...
class TestFunctionCatalog(IntegrationTestEnv):
    ...
    def test_create_get_function(self):
        ...
        function = self._catalog.as_function_catalog().register_function(
            ...
            deterministic=True,
            return_type=Types.IntegerType.get(),
            return_columns=[],
            definitions=[],
        )
        ...
        # Server may return null if not provided or supported? No, we sent it.
        # Check why return_type not returned.
        # Re-check server implementation behaviour if needed.
        # Actually for scalar function it should have return type.
