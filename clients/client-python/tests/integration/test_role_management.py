...
class TestRoleManagement(IntegrationTestEnv):
    ...
    def tearDown(self):
        try:
            self._gravitino_admin_client.drop_metalake(
                self._metalake_name, force=True
            )
    ...
    def test_grant_revoke_roles_to_user(self):
        ...
        granted = self._gravitino_client.grant_roles_to_user(
            ["user_role"], "alice"
        )
        self.assertIn("user_role", granted.roles())

        revoked = self._gravitino_client.revoke_roles_from_user(
            ["user_role"], "alice"
        )
    ...
    def test_grant_revoke_privileges_to_role(self):
        ...
        privileges = [Privileges.allow("USE_CATALOG")]
        securable_obj = SecurableObjects.of_metalake(
            self._metalake_name, privileges
        )
