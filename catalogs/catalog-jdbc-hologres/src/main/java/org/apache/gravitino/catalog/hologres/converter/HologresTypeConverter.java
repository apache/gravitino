 ...
 *
 * <p>Hologres is compatible with PostgreSQL protocol, so this converter extends PostgreSQL's type
 * converter with Hologres-specific type mappings and considerations.
 *
 * <p>Key Hologres-specific notes:
 *
 * <ul>
 *   <li>Hologres supports most PostgreSQL data types
 *   <li>Hologres has optimized storage for analytical workloads
 *   <li>Some PostgreSQL types may have limited support in Hologres
 * </ul>
 *
 * @see PostgreSqlTypeConverter
 */
public class HologresTypeConverter extends JdbcTypeConverter {

  // PostgreSQL-compatible type names (used by Hologres)
  ...
  public String fromGravitino(Type type) {
    ...
    throw new IllegalArgumentException(
        String.format("Couldn't convert Gravitino type %s to Hologres type", type.simpleString()));
  }

  /**
   * Convert Gravitino ListType to Hologres array type string.
   *
   * <p>Hologres follows PostgreSQL's array semantics:
   *
   * <ul>
   *   <li>Doesn't support multidimensional arrays internally
   *   <li>Arrays of a particular element type are all considered the same type
   *   <li>Declaring array size or dimensions is just documentation
   * </ul>
   *
   * @param listType the Gravitino list type
   * @return the Hologres array type string
   */
  private String fromGravitinoArrayType(ListType listType) {
    Type elementType = listType.elementType();
    Preconditions.checkArgument(
        !listType.elementNullable(), "Hologres doesn't support element to nullable");
    Preconditions.checkArgument(
        !(elementType instanceof ListType),
        "Hologres doesn't support multidimensional list internally, please use one dimensional list");
    String elementTypeString = fromGravitino(elementType);
    return elementTypeString + ARRAY_TOKEN;
  }

  /**
   * Convert Hologres array type to Gravitino ListType.
   *
   * @param typeName the Hologres type name (e.g., "_int4" for integer array)
   * @return the Gravitino ListType
   */
