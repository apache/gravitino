 ...
 *
 * <p>Hologres is compatible with PostgreSQL protocol and uses PostgreSQL SQLSTATE codes for error
 * reporting. This converter maps Hologres/PostgreSQL SQLSTATE codes to Gravitino exceptions.
 *
 * <p>See <a href="https://www.postgresql.org/docs/current/errcodes-appendix.html">PostgreSQL
 * errcodes appendix</a> for SQLSTATE code definitions.
 */
public class HologresExceptionConverter extends JdbcExceptionConverter {
  ...
  /**
   * SQLSTATE '08' is the class code of connection exceptions. Hologres follows PostgreSQL
   * conventions for connection error codes.
   */
  ...
  public GravitinoRuntimeException toGravitinoException(SQLException se) {
    if (null != se.getSQLState()) {
      ...
      // Handle cases where SQLState is null but we can infer the error type from the message
      if (se.getMessage() != null) {
        if (se.getMessage().contains("password authentication failed")
            || se.getMessage().contains("FATAL: password authentication failed")
            || se.getMessage().contains("FATAL: no pg_hba.conf entry")) {
          return new ConnectionFailedException(se.getMessage(), se);
        }
