 ...
 *
 * <p>Hologres is compatible with PostgreSQL protocol, so this converter follows PostgreSQL's
 * default value format with Hologres-specific considerations.
 */
public class HologresColumnDefaultValueConverter extends JdbcColumnDefaultValueConverter {

  /**
   * Pattern for matching values with type casting.
   *
   * <p>Examples:
   *
   * <ul>
   *   <li>'2021-10-01 00:00:00'::timestamp without time zone
   *   <li>'something'::character varying
   * </ul>
   */
  private static final Pattern VALUE_WITH_TYPE = Pattern.compile("'(.+?)'::\\s*(.+?)");

  /** Prefix for NULL values with type casting. */
  ...
  public Expression toGravitino(
    ...
    if (isExpression) {
      // The parsing of Hologres expressions is complex, so we are not currently undertaking the
      // parsing.
  ...
  /**
   * Parse a literal value from the database column default value.
   *
   * @param type the JDBC type bean
   * @param columnDefaultValue the column default value string
