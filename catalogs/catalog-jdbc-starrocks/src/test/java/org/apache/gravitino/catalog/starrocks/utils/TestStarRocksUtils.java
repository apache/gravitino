...
public class TestStarRocksUtils {
  ...
  public void testDistributedInfoPattern() {
    String createTableSqlWithRandomDistribution =
        "CREATE TABLE `testTable` (\n`col1` date NOT NULL\n) ENGINE=OLAP\n PARTITION BY RANGE(`col1`)\n()\n DISTRIBUTED BY RANDOM";
    Distribution distribution =
        StarRocksUtils.extractDistributionInfoFromSql(createTableSqlWithRandomDistribution);
    assertEquals(distribution.number(), -1);

    String createTableSqlWithRandomDistributionAndBucket =
        "CREATE TABLE `testTable` (\n`col1` date NOT NULL\n) ENGINE=OLAP\n PARTITION BY RANGE(`col1`)\n()\n DISTRIBUTED BY RANDOM BUCKETS 8";
    Distribution distribution2 =
        StarRocksUtils.extractDistributionInfoFromSql(
            createTableSqlWithRandomDistributionAndBucket);
    assertEquals(distribution2.number(), 8);
    ...
    assertEquals(distribution3.number(), 8);
  }
}
