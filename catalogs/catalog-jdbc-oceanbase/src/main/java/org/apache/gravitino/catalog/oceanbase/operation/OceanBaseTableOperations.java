...
public class OceanBaseTableOperations extends JdbcTableOperations {
  ...
  public static final String DEFAULT_OCEANBASE_PRIMARY_KEY_NAME = "PRIMARY";
  ...
  public static void appendIndexesSql(Index[] indexes, StringBuilder sqlBuilder) {
    for (Index index : indexes) {
      ...
      switch (index.type()) {
        case PRIMARY_KEY:
          if (null != index.name()
              && !StringUtils.equalsIgnoreCase(index.name(), DEFAULT_OCEANBASE_PRIMARY_KEY_NAME)) {
      ...
      case PRIMARY_KEY:
        if (null != addIndex.getName()
            && !StringUtils.equalsIgnoreCase(
                addIndex.getName(), DEFAULT_OCEANBASE_PRIMARY_KEY_NAME)) {
          throw new IllegalArgumentException("Primary key name must be PRIMARY in OceanBase");
        }
