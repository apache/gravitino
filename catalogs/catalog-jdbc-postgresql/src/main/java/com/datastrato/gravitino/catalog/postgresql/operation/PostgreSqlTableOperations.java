...
public class PostgreSqlTableOperations extends JdbcTableOperations {
  ...
  protected String generateCreateTableSql(
    ...
    Preconditions.checkArgument(MapUtils.isEmpty(properties), "Properties are not supported yet");
  ...
  private static String getIndexFieldStr(String[][] fieldNames) {
    return Arrays.stream(fieldNames)
        .map(
            colNames -> {
              Preconditions.checkArgument(
                  colNames.length <= 1, "Index does not support complex fields in PostgreSQL");
  ...
  private void appendColumnDefinition(JdbcColumn column, StringBuilder sqlBuilder) {
    ...
    if (column.autoIncrement()) {
      Preconditions.checkArgument(
          ...
          "Unsupported auto-increment , column: " + column.name() + ", type: " + column.dataType());
  ...
  protected String generateAlterTableSql(
    ...
    for (TableChange change : changes) {
      Preconditions.checkArgument(
          !(change instanceof TableChange.SetProperty), "Set property is not supported yet");
      Preconditions.checkArgument(
      ...
      Preconditions.checkArgument(
          ...
          "PostgreSQL does not support column position");
  ...
  private List<String> addColumnFieldDefinition(
    ...
    Preconditions.checkArgument(
        addColumn.getPosition() instanceof TableChange.Default,
