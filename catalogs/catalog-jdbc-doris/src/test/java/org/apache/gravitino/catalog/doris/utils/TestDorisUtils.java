...
public class TestDorisUtils {
  ...
  public void testDistributedInfoPattern() {
    ...
    Distribution distribution4 = DorisUtils.extractDistributionInfoFromSql(createExternalTableSql);
    assertSame(Distributions.NONE, distribution4);

    // Test case-insensitive DISTRIBUTED BY
    String createTableSqlLowerCase =
            "CREATE TABLE `testTable` (\n`col1` date NOT NULL\n) ENGINE=OLAP\n PARTITION BY RANGE(`col1`)\n()\n distributed by hash(`col1`) buckets 3";
    Distribution distribution5 = DorisUtils.extractDistributionInfoFromSql(createTableSqlLowerCase);
