 ...
 *
 * <p>Hologres is PostgreSQL-compatible, so type conversion follows PostgreSQL conventions. Hologres
...
public class HologresTypeConverter extends JdbcTypeConverter {
  ...
  // Hologres (like PostgreSQL) doesn't support the multidimensional array internally. The current
  // implementation does not enforce the declared number of dimensions either. Arrays of a
  // particular element type are all considered to be of the same type, regardless of size or number
  // of dimensions. So, declaring the array size or number of dimensions in CREATE TABLE is simply
