...
- **Naming follows existing conventions**: Match the naming patterns already used by sibling classes and the surrounding package, and make the name reflect the class's actual role. Established suffixes include `XxxManager`, `XxxDispatcher`, `XxxService`, `XxxOperations`, `XxxListener`, `XxxPoller`, capability interfaces `SupportsXxx`, and tests `TestXxx`. Capitalize acronyms consistently with existing code (`RESTUtils`, not `RestUtils`). For boolean fields and accessors, follow the surrounding module's established convention and keep the field name consistent with its getter semantics. Do not invent a new convention when a matching one already exists.
- **Names must reveal behavior**: Use domain terms that make the behavior clear. Do not name a mutating or state-transitioning method like a read-only getter (`getXxx`), and avoid vague jargon when a concrete term such as `finished`, `cleanup`, or `runnable` is clearer.
...
- **Module and package responsibilities**: Keep module and package boundaries explicit. Before adding a class, inspect sibling classes and place it in the module and package that own its concern. Do not put a class in a convenient but unrelated package, and do not bypass the owning abstraction to reach an incidental service or implementation layer.
- **Component placement & lifecycle**: A sub-component of another component (for example, a side module of the entity store) should be created, started, and closed by its owner — mirroring sibling components such as `RelationalGarbageCollector` — rather than being exposed and wired as a global `GravitinoEnv` component.
- **Avoid speculative abstractions**: Do not add an SPI, factory, helper, or extension point without a concrete current need. Prefer the smallest implementation that satisfies today's callers, then generalize when a second real use case requires it.
...
- **Argument validation**: Use `Preconditions.checkArgument(condition, message)` for argument/state validation; do not use `checkNotNull` (it throws `NullPointerException` rather than the more meaningful `IllegalArgumentException`). Only null-check values that can actually be null — do not add defensive null checks for values guaranteed non-null by their source (for example, rows returned by a MyBatis mapper).
...
- **Scheduled tasks must self-protect**: A `Runnable` scheduled on a `ScheduledExecutorService` must catch and log its own exceptions — an uncaught throwable silently cancels all future executions of that task (other scheduled tasks keep running). Advance cursors/last-run timestamps in a `finally` block so a transient failure does not cause tight-loop retries, and restore the interrupt flag on `InterruptedException`.
